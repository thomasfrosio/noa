...
namespace noa::cpu::math {
    ...
    /// \note The order in which the elements are reduced is undefined.
    ...
    template<int DDOF = 0, typename T, typename U = noa::traits::value_type_t<T>,
             typename = std::enable_if_t<details::is_valid_var_std_v<DDOF, T, U>>>
    [[nodiscard]] inline U std(const shared_t<T[]>& input, size4_t strides, size4_t shape, Stream& stream);
...
namespace noa::cpu::math {
    ...
    /// \code
    /// const size4_t input_shape{41,1,4096,4096};
    ...
    /// sum(stack.share(), input_shape.strides(), input_shape,
    ///     sum.share(), output_shape.strides(), output_shape, stream);
    /// \endcode
    template<typename T, typename = std::enable_if_t<details::is_valid_sum_mean_v<T>>>
