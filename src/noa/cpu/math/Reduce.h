...
namespace noa::cpu::math {
    ...
    /// \param stride           Rightmost strides, in elements, of \p input.
    /// \param shape            Rightmost shape of \p input.
    /// \param[out] out_sum     On the \b host. Output sum value.
    /// \param[out] out_mean    On the \b host. Output mean value.
    /// \param[out] out_var     On the \b host. Output variance value.
    /// \param[out] out_std     On the \b host. Output stddev value.
    /// \param[in,out] stream   Stream on which to enqueue this function.
    /// \note Depending on the stream, this function may be asynchronous and may return before completion.
    template<int DDOF = 0, typename T, typename U>
