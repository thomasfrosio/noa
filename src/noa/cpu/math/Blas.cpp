...
namespace {
    ...
    template<typename T>
    T cblasDot_(size_t n, const T* lhs, size_t lhs_strides, const T* rhs, size_t rhs_strides) {
        if (n >= 2048) {
            if constexpr (std::is_same_v<float, T>) {
                return static_cast<float>(cblas_dsdot(static_cast<blasint>(n),
                                                      lhs, static_cast<blasint>(lhs_strides),
                                                      rhs, static_cast<blasint>(rhs_strides)));
...
namespace noa::cpu::math {
    ...
    void dot(const std::shared_ptr<T[]>& lhs, size4_t lhs_strides, size4_t lhs_shape,
        ...
        const size_t lhs_batch_strides = lhs_strides[0];
        const size_t rhs_batch_strides = rhs_strides[0];
        stream.enqueue([=]() {
            T* ptr = output.get();
...
namespace noa::cpu::math {
    ...
    void matmul(const std::shared_ptr<T[]>& lhs, size4_t lhs_strides, size4_t lhs_shape,
        ...
        // Get the pitch:
        const blas3_t ld{lhs_strides[2 + is_col], rhs_strides[2 + is_col], output_strides[2 + is_col]};
        NOA_ASSERT(all(ld >= blas3_t{lhs_shape[3 - is_col], rhs_shape[3 - is_col], output_shape[3 - is_col]}));
        ...
        stream.enqueue([=](){
            ...
            for (size_t batch = 0; batch < output_shape[0]; ++batch) {
                cblasGEMM_(order, lhs_transpose, rhs_transpose, mnk, ld, alpha, beta,
