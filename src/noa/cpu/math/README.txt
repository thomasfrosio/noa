...
These headers extend the Math namespace for CPU arrays. These contain new functions and overloads of the
"per-element" functions declared in noa/Math.h for integer, floating-point and complex types.
...
gcc (linked with TBB), MSVC and Intel (although through non-standard headers) support them.

For now, the CPU backend is not focused on performance, so if the code is not compiled with gcc, then execution
will always be sequential, which is OK. This might change in the future (OpenMP?)...
