...
namespace noa::cpu {
    ...
    public:
        ...
        static void parallel(const Vec<Index, N>& start, const Vec<Index, N>& end, Operator&& op, i64 threads) {
    ...
    constexpr void iwise(
        ...
        if constexpr (THRESHOLD > 1) {
            ...
            if (actual_threads <= 1)
                return Iwise::parallel(start, end, std::forward<Operator>(op), actual_threads);
        }
        Iwise::serial(start, end, std::forward<Operator>(op));
    }

    template<i64 THRESHOLD = Iwise::PARALLEL_ELEMENTS_THRESHOLD, size_t N, typename Index, typename Operator>
    constexpr void launch(const Shape<Index, N>& shape, Operator&& op, i64 threads = 1) {
        iwise<THRESHOLD>(Vec<Index, N>{}, shape.vec, std::forward<Operator>(op), threads);
    }
}
#endif
