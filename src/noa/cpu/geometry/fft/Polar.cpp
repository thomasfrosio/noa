...
namespace {
    ...
    void cartesian2polar_(AccessorRestrict<const T, 3, int64_t> cartesian, long2_t cartesian_shape,
        ...
        auto interpolator = geometry::interpolator2D<BORDER_ZERO, INTERP>(cartesian, cartesian_shape.fft(), T{0});

        #pragma omp parallel for collapse(3) default(none) num_threads(threads) \
        shared(polar, polar_shape, log, interpolator, step_angle,             \
        ...
        for (dim_t batch = 0; batch < polar_shape[0]; ++batch) {
            for (dim_t phi = 0; phi < polar_shape[1]; ++phi) {
                for (dim_t rho = 0; rho < polar_shape[2]; ++rho) {
                    ...
                    T value = interpolator(cartesian_coords, batch);
                    if constexpr (traits::is_complex_v<T>)
                        value.imag *= conj;
...
namespace noa::cpu::geometry::fft {
    ...
    void cartesian2polar(const shared_t<T[]>& cartesian, dim4_t cartesian_strides, dim4_t cartesian_shape,
        ...
        const long3_t src_strides{cartesian_strides[0], cartesian_strides[2], cartesian_strides[3]};
