...
namespace noa::cpu::details {
    ...
    template<typename T>
    void permute_inplace_0213(Accessor<T, 4, isize> output, const Shape4& shape) {
    ...
    template<typename T>
    void permute_inplace_0132(Accessor<T, 4, isize> output, const Shape4& shape) {
    ...
    template<typename T>
    void permute_inplace_0321(Accessor<T, 4, isize> output, const Shape4& shape) {
...
namespace noa::cpu {
    ...
    void permute_copy(
        ...
        if (input == output) {
            using accessor_t = Accessor<T, 4, isize>;
