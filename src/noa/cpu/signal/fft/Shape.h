...
namespace noa::cpu::signal::fft::details {
    ...
    template<int32_t NDIM, Remap REMAP, typename Value, typename Matrix>
...
namespace noa::cpu::signal::fft {
    ...
    template<fft::Remap REMAP, typename Value, typename Matrix,
             typename = std::enable_if_t<details::is_valid_shape_v<3, REMAP, Value, Matrix>>>
    ...
    template<fft::Remap REMAP, typename Value, typename Matrix,
             typename = std::enable_if_t<details::is_valid_shape_v<2, REMAP, Value, Matrix>>>
    void ellipse(const shared_t<Value[]>& input, dim4_t input_strides,
                 const shared_t<Value[]>& output, dim4_t output_strides, dim4_t shape,
