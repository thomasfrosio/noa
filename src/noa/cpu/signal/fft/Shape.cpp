...
namespace {
    ...
    template<fft::Remap REMAP, typename GeomShape, typename GeomShapeSmooth,
             typename Value, typename Radius, typename Matrix>
    void launch3D_(Accessor<const Value, 4, dim_t> input,
                   Accessor<Value, 4, dim_t> output,
                   const dim4_t& start, const dim4_t& end, const dim4_t& shape,
                   const float3_t& center, const Radius& radius, float edge_size,
                   Matrix inv_matrix, dim_t threads) {
        ...
        if (has_smooth_edge) {
            ...
            if (Matrix{} == inv_matrix) {
                const auto kernel = signal::fft::details::shape3D<REMAP, dim_t>(
                        input, output, shape, geom_shape_smooth, empty_t{});
                cpu::utils::iwise4D(start, end, kernel, threads);
            } else {
                const auto kernel = signal::fft::details::shape3D<REMAP, dim_t>(
                        input, output, shape, geom_shape_smooth, inv_matrix);
                cpu::utils::iwise4D(start, end, kernel, threads);
            }
        } else {
            ...
            if (Matrix{} == inv_matrix) {
                const auto kernel = signal::fft::details::shape3D<REMAP, dim_t>(
                        input, output, shape, geom_shape, empty_t{});
                cpu::utils::iwise4D(start, end, kernel, threads);
            } else {
                const auto kernel = signal::fft::details::shape3D<REMAP, dim_t>(
                        input, output, shape, geom_shape, inv_matrix);
                cpu::utils::iwise4D(start, end, kernel, threads);
            }
        }
    }
    ...
    template<fft::Remap REMAP, typename GeomShape, typename GeomShapeSmooth,
             typename Value, typename Radius, typename Matrix>
    void launch2D_(Accessor<const Value, 3, dim_t> input,
                   Accessor<Value, 3, dim_t> output,
                   const dim4_t& start, const dim4_t& end, const dim4_t& shape,
        ...
        if (has_smooth_edge) {
            ...
            if (Matrix{} == inv_matrix) {
                const auto kernel = signal::fft::details::shape2D<REMAP, dim_t>(
                        input, output, shape_2d, geom_shape_smooth, empty_t{});
                cpu::utils::iwise3D(iwise_start, iwise_end, kernel, threads);
            } else {
                const auto kernel = signal::fft::details::shape2D<REMAP, dim_t>(
                        input, output, shape_2d, geom_shape_smooth, inv_matrix);
                cpu::utils::iwise3D(iwise_start, iwise_end, kernel, threads);
            }
        } else {
            const GeomShape geom_shape(center, radius);
            if (Matrix{} == inv_matrix) {
                const auto kernel = signal::fft::details::shape2D<REMAP, dim_t>(
                        input, output, shape_2d, geom_shape, empty_t{});
                cpu::utils::iwise3D(iwise_start, iwise_end, kernel, threads);
            } else {
                const auto kernel = signal::fft::details::shape2D<REMAP, dim_t>(
                        input, output, shape_2d, geom_shape, inv_matrix);
                cpu::utils::iwise3D(iwise_start, iwise_end, kernel, threads);
            }
        }
    }
    ...
    auto computeIwiseSubregion(const shared_t<Value[]>& input, const shared_t<Value[]>& output, const dim4_t& shape,
                               const Center& center, const Radius& radius, float edge_size, bool invert) {
        constexpr bool IS_2D = Center::COUNT == 2;
        ...
        constexpr dim_t OFFSET = 4 - Center::COUNT;
...
namespace noa::cpu::signal::fft {
    ...
    template<fft::Remap REMAP, typename Value, typename Matrix, typename>
        ...
        if (invert) {
            ...
            stream.enqueue([=]() {
                launch3D_<REMAP, ellipse_t, ellipse_smooth_t>(
                        ...
                        start, end, shape, center, radius, edge_size, inv_matrix, threads);
            });
        } else {
        ...
        if (invert) {
            ...
            stream.enqueue([=]() {
                launch3D_<REMAP, cylinder_t, cylinder_smooth_t>(
                        Accessor<const Value, 4, dim_t>(input.get(), input_strides),
                        Accessor<Value, 4, dim_t>(output.get(), output_strides),
                        start, end, shape, center, float2_t{length, radius}, edge_size, inv_matrix, threads);
            });
        } else {
    ...
    #define NOA_INSTANTIATE_SHAPE_2D_(R, T, M)                      \
    template void ellipse<R, T, M, void>(                           \
        const shared_t<T[]>&, dim4_t, const shared_t<T[]>&, dim4_t, \
        dim4_t, float2_t, float2_t, float, M, bool, Stream&);       \
    template void sphere<R, T, M, void>(                            \
        const shared_t<T[]>&, dim4_t, const shared_t<T[]>&, dim4_t, \
        dim4_t, float2_t, float, float, M, bool, Stream&);          \
    template void rectangle<R, T, M, void>(                         \
        const shared_t<T[]>&, dim4_t, const shared_t<T[]>&, dim4_t, \
        dim4_t, float2_t, float2_t, float, M, bool, Stream&)

    #define NOA_INSTANTIATE_SHAPE_3D_(R, T, M)                      \
    template void ellipse<R, T, M, void>(                           \
        const shared_t<T[]>&, dim4_t, const shared_t<T[]>&, dim4_t, \
        dim4_t, float3_t, float3_t, float, M, bool, Stream&);       \
    template void sphere<R, T, M, void>(                            \
        const shared_t<T[]>&, dim4_t, const shared_t<T[]>&, dim4_t, \
        dim4_t, float3_t, float, float, M, bool, Stream&);          \
    template void rectangle<R, T, M, void>(                         \
        const shared_t<T[]>&, dim4_t, const shared_t<T[]>&, dim4_t, \
        dim4_t, float3_t, float3_t, float, M, bool, Stream&);       \
    template void cylinder<R, T, M, void>(                          \
        const shared_t<T[]>&, dim4_t, const shared_t<T[]>&, dim4_t, \
        dim4_t, float3_t, float, float, float, M, bool, Stream&)

    #define NOA_INSTANTIATE_SHAPE_(R, T)        \
    NOA_INSTANTIATE_SHAPE_2D_(R, T, float22_t); \
    NOA_INSTANTIATE_SHAPE_2D_(R, T, float23_t); \
    NOA_INSTANTIATE_SHAPE_3D_(R, T, float33_t); \
    NOA_INSTANTIATE_SHAPE_3D_(R, T, float34_t);
