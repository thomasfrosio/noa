                ...
                if constexpr (N == 4) {
                    for (Index i = 0; i < shape[0]; ++i) {
                        for (Index j = 0; j < shape[1]; ++j) {
                            for (Index k = 0; k < shape[2]; ++k) {
                                ...
                                for (Index l = 0; l < shape[3]; ++l) {
                                    if constexpr (R == 0) {
                                        interface::init(op_, local, l, i, j, k);
                                    } else if constexpr (R == 1) {
                                        interface::init(op_, local, i, l, j, k);
                                    } else if constexpr (R == 2) {
                                        interface::init(op_, local, i, j, l, k);
                                    } else if constexpr (R == 3) {
                                        interface::init(op_, local, i, j, k, l);
                                    } else {
                                        static_assert(nt::always_false_v<Index>);
                                    }
                                }
                                interface::final(op_, local, output, i, j, k);
                            }
                        }
                    }
                } else if constexpr (N == 3) {
                    for (Index i = 0; i < shape[0]; ++i) {
                        for (Index j = 0; j < shape[1]; ++j) {
                            ...
                            for (Index k = 0; k < shape[2]; ++k) {
                                if constexpr (R == 0) {
                                    interface::init(op_, local, k, i, j);
                                } else if constexpr (R == 1) {
                                    interface::init(op_, local, i, k, j);
                                } else if constexpr (R == 2) {
                                    interface::init(op_, local, i, j, k);
                                } else {
                                    static_assert(nt::always_false_v<Index>);
                                }
                            }
                            interface::final(op_, local, output, i, j);
                        }
                    }
                } else if constexpr (N == 2) {
                    for (Index i = 0; i < shape[0]; ++i) {
                        ...
                        for (Index j = 0; j < shape[1]; ++j) {
                            if constexpr (R == 0) {
                                interface::init(op_, local, j, i);
                            } else {
                                static_assert(nt::always_false_v<Index>);
                            }
                        }
                        interface::final(op_, local, output, i);
                    }
                } else {
                    static_assert(nt::always_false_v<Index>);
                }
...
namespace noa::cpu {
    ...
    constexpr void reduce_axes_iwise(
        ...
        if (all(axes_empty_or_to_reduce)) { // reduce to a single value
            ...
            return reduce_iwise<Config>(input_shape, op, reduced, output_1d, n_threads);
        }
        ...
        const bool is_small = n_elements_to_reduce <= Config::parallel_threshold;
        ...
        if constexpr (N == 4) {
            if (all(axes_empty_or_to_reduce.pop_front())) { // reduce to one value per batch
                ...
                if (is_small or n_threads <= 1) {
                    reducer::serial_4d(input_shape, op, reduced, output_1d);
                } else if (n_batches < n_threads) {
                    reducer::template parallel_4d<reducer::ParallelReduction>(input_shape, op, reduced, output_1d, n_threads);
                } else {
                    reducer::template parallel_4d<reducer::SerialReduction>(input_shape, op, reduced, output_1d, n_threads);
                }
            } else {
                if (axes_to_reduce[3]) {
            ...
            if (all(axes_empty_or_to_reduce.pop_front())) { // reduce to one value per batch
                auto output_1d = ng::reconfig_accessors<ng::AccessorConfig<1>{.filter={0}}>(output);
                if (is_small or n_threads <= 1) {
                    reducer::serial_3d(input_shape, op, reduced, output_1d);
                } else if (n_batches < n_threads) {
                    reducer::template parallel_3d<reducer::ParallelReduction>(input_shape, op, reduced, output_1d, n_threads);
                } else {
                    reducer::template parallel_3d<reducer::SerialReduction>(input_shape, op, reduced, output_1d, n_threads);
                }
            } else {
                if (axes_to_reduce[2]) {
            ...
            if (axes_to_reduce[1]) {
                if (is_small or n_threads <= 1) {
                    ...
                    reducer::template parallel_2d<reducer::ParallelReduction>(input_shape, op, reduced, output_1d, n_threads);
                } else {
