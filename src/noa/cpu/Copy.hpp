...
namespace noa::cpu {
    ...
    void copy(const T* src, T* dst, i64 elements) {
        NOA_ASSERT(!elements || (src && dst && !noa::are_overlapped(src, elements, dst, elements)));
    ...
    void copy(
        ...
        NOA_ASSERT(noa::all(shape > 0) && src && dst && !noa::are_overlapped(src, src_strides, dst, dst_strides, shape));
        ewise_unary(src, src_strides, dst, dst_strides, shape, noa::copy_t{}, threads);
    }
}
#endif
