...
namespace noa::cpu {
    ...
    void copy(const T* src, T* dst, i64 elements) {
        NOA_ASSERT(!elements || (src && dst && !ni::are_overlapped(src, elements, dst, elements)));
    ...
    void copy(
        ...
        NOA_ASSERT(all(shape > 0) && src && dst && !ni::are_overlapped(src, src_strides, dst, dst_strides, shape));
         ewise(wrap(AccessorRestrictI64<const T, 4>(src, src_strides)),
               wrap(AccessorRestrictI64<T, 4>(dst, dst_strides)),
               shape, Copy{}, threads);
    }
}
#endif
