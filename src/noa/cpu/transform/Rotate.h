...
namespace noa::cpu::transform {
    ...
    NOA_IH void rotate2D(const T* input, T* outputs, size2_t shape,
        ...
        if (nb_rotations == 1U) { // allocate only if necessary
            float23_t inv_transform(noa::transform::translate(rotation_centers[0]) *
            ...
            for (uint i = 0; i < nb_rotations; ++i)
                inv_transforms[i] = float23_t(noa::transform::translate(rotation_centers[i]) *
    ...
    NOA_HOST void rotate2D(const T* input, T* output, size2_t shape,
                           ...
                           InterpMode interp_mode, BorderMode border_mode, T value) {
        float23_t inv_transform(noa::transform::translate(rotation_center) *
    ...
    NOA_IH void rotate3D(const T* input, T* outputs, size3_t shape,
                         ...
                         InterpMode interp_mode, BorderMode border_mode, T value) {
        if (nb_rotations == 1U) { // allocate only if necessary
            float34_t inv_transform(noa::transform::translate(rotation_centers[0]) *
            ...
            for (uint i = 0; i < nb_rotations; ++i)
                inv_transforms[i] = float34_t(noa::transform::translate(rotation_centers[i]) *
    ...
    NOA_HOST void rotate3D(const T* input, T* output, size3_t shape,
                           ...
                           InterpMode interp_mode, BorderMode border_mode, T value) {
        float34_t inv_transform(noa::transform::translate(rotation_center) *
