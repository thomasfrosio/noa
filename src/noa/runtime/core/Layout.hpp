...
namespace noa {
    ...
    [[nodiscard]] NOA_HD constexpr bool are_contiguous(
        const Strides<T, 4>& strides,
    ...
    template<char ORDER = 'C', typename T>
    [[nodiscard]] NOA_HD constexpr auto is_contiguous(
        Strides<T, 4> strides,
        const Shape<T, 4>& shape
    ) noexcept {
        ...
        strides *= Strides<T, 4>::from_vec(shape.cmp_ne(1)); // mark the stride of empty dimensions unusable
    ...
    template<typename T, usize N> requires (N <= 3)
    ...
    template<typename T, usize N>
    [[nodiscard]] NOA_HD constexpr auto rightmost_order(Strides<T, N> strides, const Shape<T, N>& shape) noexcept {
        Vec<T, N> order;
        for (usize i{}; i < N; ++i) {
    ...
    template<typename T, nt::integer I, usize N> requires nt::vec_shape_or_strides_of_size<T, N>
    ...
    template<typename T, usize N>
    constexpr bool is_broadcastable(const Shape<T, N>& input, const Shape<T, N>& output) {
...
namespace noa::details {
    ...
    [[nodiscard]] constexpr auto are_elements_unique(
        ...
        if (rightmost_order != Vec<T, N>::arange()) {
    ...
    public:
        ...
        [[nodiscard]] auto as() const {
            ...
            if constexpr (nt::is_almost_same_v<old_type, New> or
                ...
                const vec_type rightmost_order = noa::rightmost_order(out.strides, out.shape);
