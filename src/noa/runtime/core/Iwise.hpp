    ...
    public:
        ...
        NOA_HD constexpr void operator()(U... indices) const {
            if constexpr (CONTIGUOUS_1D)
                ...
                m_output(indices...) = static_cast<value_type>(m_range(noa::offset_at(m_contiguous_strides, indices...)));
        }
    ...
    public:
        ...
        NOA_HD constexpr void operator()(const indices_type& indices) const {
            if constexpr (CONTIGUOUS_1D) {
                ...
                const auto iota = noa::offset_at(m_contiguous_strides, indices % m_tile);
