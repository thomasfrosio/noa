    ...
    public:
        ...
        constexpr void operator()(const index4_type& output_indices) const {
            ...
            if constexpr (MODE == Border::NOTHING) {
                ...
                m_subregions(output_indices) = is_inbound(m_input_shape, input_indices) ?
                ...
                const index4_type bounded_indices = index_at<MODE>(input_indices, m_input_shape);
...
namespace noa {
    ...
    void extract_subregions(
        ...
        check(is_contiguous_vector(origins) and origins.n_elements() == subregions.shape()[0],
    ...
    auto atlas_layout(const Shape4& subregion_shape, const Origin& output_origins) -> Shape4 {
        ...
        check(output_origins.device().is_cpu() and
              is_contiguous_vector(output_origins) and
