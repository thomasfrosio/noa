...
namespace noa::guts {
    ...
    template<typename... Ts> struct proclaim_is_adaptor<AdaptorZip<Ts...>> : std::true_type {};
    template<typename... Ts> struct proclaim_is_adaptor<AdaptorUnzip<Ts...>> : std::true_type {};
    ...
    template<typename... Ts> using are_adaptor = nt::bool_and<is_adaptor<Ts>::value...>;
...
namespace noa {
    ...
    constexpr auto zip(T&&... a) noexcept {
        return guts::AdaptorZip<T&&...>{.tuple=forward_as_tuple(std::forward<T>(a)...)};
    }

    template<typename... T>
    constexpr auto wrap(T&&... a) noexcept {
        return guts::AdaptorUnzip<T&&...>{.tuple=forward_as_tuple(std::forward<T>(a)...)};
    }
}
