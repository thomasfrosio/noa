...
namespace {
    ...
    inline void serialize_(const Input* input, Byte* output, i64 elements,
        ...
        if constexpr (std::is_same_v<Input, Output>) {
            ...
            std::memcpy(output, reinterpret_cast<const Byte*>(input), static_cast<size_t>(elements * OUTPUT_SIZE));
        } else {
            Output tmp;
            if (clamp) {
                for (i64 idx = 0; idx < elements; ++idx) {
    ...
    template<typename Output, typename Input>
    inline void serialize_(const Input* input, const Strides4<i64>& strides, const Shape4<i64>& shape,
                           Byte* output, bool clamp, bool swap_endian) {
        ...
        if (clamp) {
            for (i64 i = 0; i < shape[0]; ++i) {
                for (i64 j = 0; j < shape[1]; ++j) {
                    for (i64 k = 0; k < shape[2]; ++k) {
                        for (i64 l = 0; l < shape[3]; ++l, ++idx) {
                            tmp = clamp_cast<Output>(input[indexing::at(i, j, k, l, strides)]);
            ...
            for (i64 i = 0; i < shape[0]; ++i) {
                for (i64 j = 0; j < shape[1]; ++j) {
                    for (i64 k = 0; k < shape[2]; ++k) {
                        for (i64 l = 0; l < shape[3]; ++l, ++idx) {
                            tmp = static_cast<Output>(input[indexing::at(i, j, k, l, strides)]);
                            std::memcpy(output + idx * OUTPUT_SIZE, &tmp, sizeof(Output));
                        }
                    }
                }
    ...
    inline void deserialize_(const Byte* input, Output* output, i64 elements,
                             bool clamp, bool swap_endian) {
        if constexpr (std::is_same_v<Output, Input>) {
            auto* output_ptr = reinterpret_cast<Byte*>(output);
            ...
            for (i64 idx = 0; idx < elements; ++idx) {
                ...
                if (swap_endian)
                    io::details::reverse<sizeof(Input)>(reinterpret_cast<Byte*>(&tmp));
    ...
    template<typename Input, typename Output>
    inline void deserialize_(const Byte* input, Output* output,
            ...
            for (i64 j = 0; j < shape[1]; ++j) {
                for (i64 k = 0; k < shape[2]; ++k) {
                    for (i64 l = 0; l < shape[3]; ++l, ++idx) {
                        ...
                        if (swap_endian)
                            io::details::reverse<sizeof(Input)>(reinterpret_cast<Byte*>(&tmp));
                        output[indexing::at(i, j, k, l, strides)] =
    ...
    void serialize_row_4bits_(const T* input, Byte* output, i64 elements_row, bool is_odd, bool clamp) {
        ...
        if (clamp) {
            ...
            for (i64 i = 0; i < elements_row / 2; ++i) {
                l_val = static_cast<u32>(noa::math::round(input[2 * i]));
                h_val = static_cast<u32>(noa::math::round(input[2 * i + 1] ));
                tmp = l_val + (h_val << 4);
                std::memcpy(output + i, &tmp, 1);
            }
            if (is_odd) {
                l_val = static_cast<u32>(noa::math::round(input[elements_row - 1]));
                std::memcpy(output + elements_row / 2, &l_val, 1);
            }
        }
    }
    ...
    template<typename T>
    inline void deserialize_row_4bits_(const Byte* input, T* output, i64 elements_row, bool is_odd) {
...
namespace noa::io {
    ...
    void serialize(const Input* input, const Strides4<i64>& strides, const Shape4<i64>& shape,
        ...
        NOA_THROW("{} cannot be serialized into the data type {}", string::human<Input>(), data_type);
    }
    ...
    void serialize(const void* input, DataType input_data_type,
        ...
        switch (input_data_type) {
            case DataType::I8:
                return serialize(reinterpret_cast<const i8*>(input), strides, shape,
            ...
            case DataType::I32:
                return serialize(reinterpret_cast<const i32*>(input), strides, shape,
                                 ...
                                 clamp, swap_endian);
            case DataType::U32:
                return serialize(reinterpret_cast<const u32*>(input), strides, shape,
                                 output, output_data_type,
                                 clamp, swap_endian);
            case DataType::I64:
                return serialize(reinterpret_cast<const i64*>(input), strides, shape,
                                 output, output_data_type,
                                 clamp, swap_endian);
            case DataType::U64:
    ...
    template<typename T>
    void serialize(const T* input, const Strides4<i64>& strides, const Shape4<i64>& shape,
        ...
        const bool are_contiguous = indexing::are_contiguous(strides, shape);
        ...
        if (are_contiguous && !swap_endian && data_type == dtype<T>()) {
            ...
            const i64 bytes = serialized_size(DataType::U4, elements, shape[3]);
            const auto buffer = std::make_unique<Byte[]>(static_cast<size_t>(bytes));
            ...
            const i64 bytes_per_element = serialized_size(data_type, 1);
            i64 bytes_remain = bytes_per_element * elements;
            i64 bytes_buffer = bytes_remain > bytes_per_batch ? bytes_per_batch : bytes_remain;
            ...
            for (; bytes_remain > 0; bytes_remain -= bytes_buffer) {
                ...
                const i64 elements_buffer = bytes_buffer / bytes_per_element;
                const Shape4<i64> buffer_shape{1, 1, 1, elements_buffer};
            ...
            const i64 elements_per_slice = shape[2] * shape[3];
            const i64 bytes_per_slice = serialized_size(data_type, elements_per_slice);
            const Shape4<i64> slice_shape{1, 1, shape[2], shape[3]};
            const Strides4<i64> slice_strides{0, 0, strides[2], strides[3]};
            const auto buffer = std::make_unique<Byte[]>(static_cast<size_t>(bytes_per_slice));
            const auto* buffer_ptr = reinterpret_cast<const char*>(buffer.get());
...
namespace noa::io {
    ...
    void deserialize(const Byte* input, DataType input_data_type,
        ...
        switch (output_data_type) {
            ...
            case DataType::U32:
                return deserialize(input, input_data_type,
                                   static_cast<u32*>(output), strides,
                                   shape, clamp, swap_endian);
            case DataType::I64:
                return deserialize(input, input_data_type,
                                   static_cast<i64*>(output), strides,
                                   shape, clamp, swap_endian);
            case DataType::U64:
                return deserialize(input, input_data_type,
