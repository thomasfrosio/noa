...
namespace noa::fft::details {
    ...
    public:
        ...
        constexpr void operator()(const Vec<index_type, B + R>& output_batched_indices) const {
            ...
            auto& o = m_output[batches.push_back(output_indices)];
            auto& i = m_output[batches.push_back(input_indices)];
    ...
    public:
        ...
        constexpr void operator()(const Vec<index_type, B + R>& output_batched_indices) const {
            ...
            if constexpr (REMAP == Layout::HC2H) {
                ...
                const auto input_indices = output_indices.template set<R - 1>(
                    fftshift(output_indices[R - 1], m_shape[R - 1]));
                output[output_indices] = cast_or_abs_squared<output_value_type>(input[input_indices]);
                ...
                if (output_indices[R - 1] < m_shape[R - 1] / 2 + 1) {
                    ...
                    if constexpr (R > 1)
                        for (usize i = 0; i < R - 1; ++i)
                            input_indices[i] = fftshift(
                                output_indices[i] != 0 ? m_shape[i] - output_indices[i] : output_indices[i], m_shape[i]);
    ...
    template<usize R, usize N, typename Input, typename Output>
    concept remapable =
...
namespace noa::fft {
    ...
    // TODO Add overload with REMAP at compile time. This would save the number of instantiations.
    ...
    /// \param remap:
    ///     Remapping operation.
    ///     If h2hc, the input can be equal to the output, iff the height and depth are even or 1.
    /// \param[in] input:
    ///     Input FFT to remap.
    ...
    ///     Remapped FFT.
    ///     ((B..,)R), where R is W (RANK=1), HW (RANK=2), or DHW (RANK=3).
    ///     Input values are cast to the output type, or if the input is real and the output is complex,
    ///     the power spectrum is saved.
    /// \param shape:
    ///     Logical shape of the input and output.
    /// \param options:
    ///     Remap options.
    ///
    /// \bug The rfft<->fft remappings, e.g. h2f, should be used with care. If the input has an anisotropic field, with
    ///      an anisotropic angle that is not a multiple of pi/2, and has even-sized dimensions, the remapping will
    ///      not be correct for the real-Nyquist frequencies, except the ones on the DHW axes. This situation is rare
    ///      but can happen if a geometric scaling factor is applied to the input, or with CTFs with astigmatic
    ...
    ///      filtering to Nyquist (fftfreq=0.5) fixes this issue. See noa/docs/040_fft_layouts.md.
    template<usize RANK = 0, typename Input, typename Output, usize N>
        requires details::remapable<RANK, N, Input, Output>
    void remap(Layout remap, Input&& input, Output&& output, const Shape<isize, N>& shape, RemapOptions options = {}) {
        ...
        check(input.shape() == (remap.is_fx2xx() ? shape : shape.rfft()),
              "Given the {} remap, the input spectrum is expected to have a physical shape of {}, but got {}",
              remap, remap.is_fx2xx() ? shape : shape.rfft(), input.shape());
        check(output.shape() == (remap.is_xx2fx() ? shape : shape.rfft()),
              "Given the {} remap, the output spectrum is expected to have a physical shape of {}, but got {}",
        ...
        if (not remap.has_layout_change()) {
            if constexpr (nt::same_as<input_value_t, output_value_t>) {
                if (not is_inplace)
                    copy(NOA_FWD(input), NOA_FWD(output));
            } else {
                cast(NOA_FWD(input), NOA_FWD(output));
            }
            return;
        }
        ...
        const auto shape_r = shape_br.template pop_front<B>();
        ...
        if (is_inplace) {
            ...
            // TODO check if the width needs to be even.
            bool valid{true};
            for (auto e: shape_r)
                if (e != 1 and noa::is_odd(e))
                    valid = false;
            check(valid,
                  "In-place remapping requires the ranked axes to have an even number of elements, but got shape_r={}",
                  shape_r);
        }
        ...
        switch (remap) {
            ...
            case Layout::H2HC: {
                if (is_inplace) {
                    auto op = FourierRemapInplace<Layout::H2HC, B, R, isize, output_t>(oaccessor, shape_r);
                    return iwise(iwise_shape, device, op, std::forward<Input>(input), std::forward<Output>(output));
                }
                auto op = FourierRemap<Layout::H2HC, B, R, isize, input_t, output_t>(iaccessor, oaccessor, shape_r);
                return iwise(iwise_shape, device, op, std::forward<Input>(input), std::forward<Output>(output));
            }
            case Layout::HC2H: {
                if (is_inplace) {
                    auto op = FourierRemapInplace<Layout::HC2H, B, R, isize, output_t>(oaccessor, shape_r);
                    return iwise(iwise_shape, device, op, std::forward<Input>(input), std::forward<Output>(output));
                }
                auto op = FourierRemap<Layout::HC2H, B, R, isize, input_t, output_t>(iaccessor, oaccessor, shape_r);
                return iwise(iwise_shape, device, op, std::forward<Input>(input), std::forward<Output>(output));
            }
            case Layout::H2F: {
                auto op = FourierRemap<Layout::H2F, B, R, isize, input_t, output_t>(iaccessor, oaccessor, shape_r);
                return iwise(iwise_shape, device, op, std::forward<Input>(input), std::forward<Output>(output));
            }
            case Layout::F2H: {
                auto op = FourierRemap<Layout::F2H, B, R, isize, input_t, output_t>(iaccessor, oaccessor, shape_r);
                return iwise(iwise_shape, device, op, std::forward<Input>(input), std::forward<Output>(output));
            }
            case Layout::F2FC: {
                auto op = FourierRemap<Layout::F2FC, B, R, isize, input_t, output_t>(iaccessor, oaccessor, shape_r);
                return iwise(iwise_shape, device, op, std::forward<Input>(input), std::forward<Output>(output));
            }
            case Layout::FC2F: {
                auto op = FourierRemap<Layout::FC2F, B, R, isize, input_t, output_t>(iaccessor, oaccessor, shape_r);
                return iwise(iwise_shape, device, op, std::forward<Input>(input), std::forward<Output>(output));
            }
            case Layout::HC2F: {
                auto op = FourierRemap<Layout::HC2F, B, R, isize, input_t, output_t>(iaccessor, oaccessor, shape_r);
                return iwise(iwise_shape, device, op, std::forward<Input>(input), std::forward<Output>(output));
            }
            case Layout::F2HC: {
                auto op = FourierRemap<Layout::F2HC, B, R, isize, input_t, output_t>(iaccessor, oaccessor, shape_r);
                return iwise(iwise_shape, device, op, std::forward<Input>(input), std::forward<Output>(output));
            }
            case Layout::FC2H: {
                auto op = FourierRemap<Layout::FC2H, B, R, isize, input_t, output_t>(iaccessor, oaccessor, shape_r);
                return iwise(iwise_shape, device, op, std::forward<Input>(input), std::forward<Output>(output));
            }
            case Layout::FC2HC: {
                auto op = FourierRemap<Layout::FC2HC, B, R, isize, input_t, output_t>(iaccessor, oaccessor, shape_r);
                return iwise(iwise_shape, device, op, std::forward<Input>(input), std::forward<Output>(output));
            }
            case Layout::HC2FC: {
                auto op = FourierRemap<Layout::HC2FC, B, R, isize, input_t, output_t>(iaccessor, oaccessor, shape_r);
                return iwise(iwise_shape, device, op, std::forward<Input>(input), std::forward<Output>(output));
            }
            case Layout::H2FC: {
                auto op = FourierRemap<Layout::H2FC, B, R, isize, input_t, output_t>(iaccessor, oaccessor, shape_r);
                return iwise(iwise_shape, device, op, std::forward<Input>(input), std::forward<Output>(output));
            }
        }
    }
    ...
    /// Remaps fft(s).
    template<usize RANK = 0, typename Input, usize N>
