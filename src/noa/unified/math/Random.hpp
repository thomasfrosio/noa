...
namespace noa::math::details {
    ...
    constexpr bool is_valid_random_v =
            ...
            (noa::traits::is_scalar_v<U> || noa::traits::are_complex_v<T, U>);

    template<typename T, typename U>
    auto cast_to_supported_type(U value) {
        if constexpr (noa::traits::are_complex_v<T, U>) {
            using supported_t = std::conditional_t<noa::traits::is_almost_same_v<T, c16>, c32, T>;
            return static_cast<supported_t>(value);
        } else {
            using real_t = noa::traits::value_type_t<T>;
            using supported_t = std::conditional_t<noa::traits::is_almost_same_v<real_t, f16>, f32, real_t>;
            return static_cast<supported_t>(value);
        }
    }
}
...
namespace noa::math {
    ...
    template<typename T, typename Real,
             typename = std::enable_if_t<details::is_valid_random_v<T, Real> && noa::traits::is_real_v<Real>>>
    [[nodiscard]] Array<T> random(
            noa::math::poisson_t,
            const Shape4<i64>& shape,
            Real lambda,
