...
namespace noa::io {
    ...
    /// \return             Output array containing the whole data array of the file.
    /// \note This is an utility function. For better flexibility, use ImageFile directly.
    template<typename T>
    Array<T> load(const path_t& filename, ArrayOption option = {}, bool clamp = false) {
        ImageFile file(filename, io::READ);
        if (option.dereferencable()) {
