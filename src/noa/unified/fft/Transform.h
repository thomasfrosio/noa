...
namespace noa::fft {
    ...
    /// Returns the next optimum rightmost shape.
    /// \note Dimensions of size 0 or 1 are ignored as well as the leftmost dimension, e.g. {1,1,53,53}
    ///       is rounded up to {1,1,54,54}. The batch dimension is also ignored.
    template<typename T>
    ...
    /// \param shape        Rightmost logical shape of \p input.
    ...
    ///                     It can be −1 (\c fft::FORWARD) or +1 (\c fft::BACKWARD).
    /// \param norm         Normalization mode.
    /// \return output Non-centered FFT(s).
    template<typename T, typename = std::enable_if_t<traits::is_any_v<T, float, double>>>
