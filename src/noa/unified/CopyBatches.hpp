...
namespace noa::memory {
    ...
    template<typename Input, typename Output, typename Indexes>
    requires (nt::are_varray_v<Input, Output> and
              nt::are_almost_same_value_type_v<Input, Output> and
              nt::is_varray_of_almost_any_v<Indexes, i32, i64>)
    void copy_batches(
            const Input& input, const Output& output,
            const Indexes& batch_indexes,
            i64 group_copy_at_count = 3
    ) {
        check(not input.is_empty() and not output.is_empty() and not batch_indexes.is_empty(), "Empty array detected");
        ...
        check(ni::is_contiguous_vector(batch_indexes) and output.shape()[0] == batch_indexes.elements(),
        ...
        check(batch_indexes.device().is_cpu(),
        ...
        index_t index{};
        for (i64 i = 0; i < n_batches_to_copy; ++i, ++index) {
        ...
        if (index > 0) {
            NOA_ASSERT(batch_indexes_1d[0] + n_batches_to_copy == index);
            const auto slice = ni::Slice{batch_indexes_1d[0], index};
        ...
        if (input.device() == device and n_batches_to_copy >= group_copy_at_count) {
            ...
            for (size_t i = 0; i < n_batches_to_copy; ++i)
            ...
            return extract_subregions(input, output, batch_origins, Border::NOTHING);
        }

        // Worst case, copy batches one by one across devices.
        for (i64 i = 0; i < n_batches_to_copy; ++i)
