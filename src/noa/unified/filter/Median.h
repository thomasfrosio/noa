...
namespace noa::filter {
    ...
    void median1(const Array<T>& input, const Array<T>& output,
        ...
        if (device.cpu()) {
            cpu::filter::median1(input.share(), input_stride,
                                 output.share(), output.stride(), output.shape(),
                                 border_mode, window_size, stream.cpu());
        } else {
            #ifdef NOA_ENABLE_CUDA
            cuda::filter::median1(input.share(), input_stride,
                                  output.share(), output.stride(), output.shape(),
    ...
    void median2(const Array<T>& input, const Array<T>& output,
        ...
        Stream& stream = Stream::current(device);
        if (device.cpu()) {
            cpu::filter::median2(input.share(), input_stride,
                                 output.share(), output.stride(), output.shape(),
                                 border_mode, window_size, stream.cpu());
        } else {
            #ifdef NOA_ENABLE_CUDA
            cuda::filter::median2(input.share(), input_stride,
                                  output.share(), output.stride(), output.shape(),
                                  border_mode, window_size, stream.cuda());
            #else
            NOA_THROW("No GPU backend detected");
    ...
    void median3(const Array<T>& input, const Array<T>& output,
        ...
        Stream& stream = Stream::current(device);
        if (device.cpu()) {
            cpu::filter::median3(input.share(), input_stride,
                                 output.share(), output.stride(), output.shape(),
                                 border_mode, window_size, stream.cpu());
        } else {
            #ifdef NOA_ENABLE_CUDA
            cuda::filter::median3(input.share(), input_stride,
                                  output.share(), output.stride(), output.shape(),
                                  border_mode, window_size, stream.cuda());
            #else
            NOA_THROW("No GPU backend detected");
