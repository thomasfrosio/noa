...
namespace noa::signal::details {
    constexpr dim_t CUDA_FILTER_MAX_BYTES = 1032;
}
#endif
...
namespace noa::signal {
    ...
    void convolve(const Array<T>& input, const Array<T>& output, const Array<T>& filter) {
        NOA_CHECK(!input.empty() && !output.empty() && !filter.empty(), "Empty array detected");
        ...
        const dim4_t filter_shape = filter.shape();
        dim_t ndim = dim3_t(output.shape().get(1)).ndim();
        ...
        if (device.cpu()) {
            ...
            cpu::signal::convolve(input.share(), input_strides,
                                  ...
                                  filter.share(), dim3_t(filter_shape.get(1)), stream.cpu());
        } else {
            #ifdef NOA_ENABLE_CUDA
                ...
                cuda::signal::convolve(input.share(), input_strides,
                                       output.share(), output.strides(), output.shape(),
                                       filter.share(), dim3_t(filter_shape.get(1)), stream.cuda());
            #else
    ...
    void convolve(const Array<T>& input, const Array<T>& output,
        ...
        for (auto i: irange<dim_t>(3)) {
            ...
            NOA_CHECK(indexing::isVector(filter.shape()) && filter.elements() % 2 == 1,
        ...
        if (device.cpu()) {
            cpu::signal::convolve(input.share(), input_strides,
                                  output.share(), output.strides(), output.shape(),
                                  filter1.share(), filter1.elements(),
                                  filter2.share(), filter2.elements(),
                                  filter3.share(), filter3.elements(),
                                  stream.cpu(), tmp.share(), tmp.strides());
        } else {
            #ifdef NOA_ENABLE_CUDA
                NOA_CHECK(filter1.elements() * sizeof(T) <= details::CUDA_FILTER_MAX_BYTES &&
                          filter2.elements() * sizeof(T) <= details::CUDA_FILTER_MAX_BYTES &&
                          filter3.elements() * sizeof(T) <= details::CUDA_FILTER_MAX_BYTES,
