...
namespace noa::xform {
    ...
    /// \param log_polar        Whether this is the log-polar coordinates system.
    template<nt::real T, nt::sinteger I, usize A>
    [[nodiscard]] NOA_IHD auto polar2rho(
        T polar_coordinate,
        I polar_size,
        const Vec<T, 2, A>& radius_range,
        bool log_polar
    ) -> T {
    ...
    template<nt::real T, nt::sinteger I, usize A>
    [[nodiscard]] NOA_IHD auto polar2phi(T coordinate, I size, const Vec<T, 2, A>& angle_range) -> T {
    ...
    [[nodiscard]] NOA_IHD auto polar2cartesian(
        ...
        const T rho = polar2rho(polar_coordinate[1], polar_shape[1], radius_range, log_polar);
...
namespace noa::xform {
    ...
    /// \param log_polar    Whether this is the log-polar coordinates system.
    template<nt::real T, nt::sinteger I, usize A>
    [[nodiscard]] NOA_IHD constexpr auto rho2polar(
        T rho,
        I polar_size,
        const Vec<T, 2, A>& radius_range,
        bool log_polar
    ...
    [[nodiscard]] NOA_IHD auto cartesian2polar(
        ...
        return {phi2polar(phi, polar_shape[0], angle_range),
                rho2polar(rho, polar_shape[1], radius_range, log_polar)};
    }
}
