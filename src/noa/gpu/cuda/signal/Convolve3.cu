...
namespace {
    ...
    constexpr int32_t MAX_FILTER_SIZE = 5;
    constexpr int32_t MAX_FILTER_BYTES = MAX_FILTER_SIZE * MAX_FILTER_SIZE * MAX_FILTER_SIZE * sizeof(double);
    __constant__ char cfilter[MAX_FILTER_BYTES];

    template<typename T, int32_t FILTER_LEN>
    __global__ __launch_bounds__(BLOCK_SIZE.x * BLOCK_SIZE.y)
    void convolve3Square_(AccessorRestrict<const T, 4, uint32_t> input,
                          AccessorRestrict<T, 4, uint32_t> output,
                          uint3_t shape, uint32_t blocks_x) {
        static_assert(FILTER_LEN % 2); // only support odd windows.
        constexpr int32_t PADDING = FILTER_LEN - 1; // assume odd
        constexpr int32_t HALO = FILTER_LEN / 2;
        ...
        for (int32_t lz = 0, gz = gid[1]; lz < SHARED_LEN[0]; ++lz, ++gz) {
            ...
            for (int32_t ly = tid[0], gy = gid[2]; ly < SHARED_LEN[1]; ly += BLOCK_SIZE.y, gy += BLOCK_SIZE.y) {
                const int32_t i_y = gy - HALO;
                const int32_t tmp = tmp_z + ly * SHARED_LEN[2];
                const bool is_in_y = i_y >= 0 && i_y < shape[1];
                for (int32_t lx = tid[1], gx = gid[3]; lx < SHARED_LEN[2]; lx += BLOCK_SIZE.x, gx += BLOCK_SIZE.x) {
                    const int32_t i_x = gx - HALO;
                    const bool is_in_x = i_x >= 0 && i_x < shape[2];
                    shared[tmp + lx] = (is_in_z && is_in_y && is_in_x) ? input_(i_z, i_y, i_x) : T{0};
                }
            }
        }
        ...
        if (gid[2] < shape[1] && gid[3] < shape[2]) {
            ...
            T result{0};
            for (int32_t z = 0; z < FILTER_LEN; ++z)
                for (int32_t y = 0; y < FILTER_LEN; ++y)
                    for (int32_t x = 0; x < FILTER_LEN; ++x)
    ...
    template<typename T>
    __global__ __launch_bounds__(BLOCK_SIZE.x * BLOCK_SIZE.y)
    void convolve3_(AccessorRestrict<const T, 4, uint32_t> input,
                    AccessorRestrict<T, 4, uint32_t> output,
                    uint3_t shape, int3_t filter_length, uint32_t blocks_x) {
        ...
        for (int32_t lz = 0, gz = gid[1]; lz < shared_len[0]; ++lz, ++gz) {
            ...
            const bool is_in_z = i_z >= 0 && i_z < shape[0];
            for (int32_t ly = tid[0], gy = gid[2]; ly < shared_len[1]; ly += BLOCK_SIZE.y, gy += BLOCK_SIZE.y) {
                const int32_t i_y = gy - halo[1];
                const int32_t tmp = tmp_z + ly * shared_len[2];
                const bool is_in_y = i_y >= 0 && i_y < shape[1];
                for (int32_t lx = tid[1], gx = gid[3]; lx < shared_len[2]; lx += BLOCK_SIZE.x, gx += BLOCK_SIZE.x) {
                    const int32_t i_x = gx - halo[2];
                    const bool is_in_x = i_x >= 0 && i_x < shape[2];
                    shared[tmp + lx] = (is_in_z && is_in_y && is_in_x) ? input_(i_z, i_y, i_x) : T{0};
                }
            }
        ...
        if (gid[2] < shape[1] && gid[3] < shape[2]) {
            ...
            const T* window = reinterpret_cast<T*>(cfilter);
            T result{0};
            for (int32_t z = 0; z < filter_length[0]; ++z)
                for (int32_t y = 0; y < filter_length[1]; ++y)
                    for (int32_t x = 0; x < filter_length[2]; ++x)
                        result += shared[(z * shared_len[1] + tid[0] + y) * shared_len[2] + tid[1] + x] *
...
namespace noa::cuda::signal {
    ...
    void convolve3(const shared_t<T[]>& input, dim4_t input_strides,
        ...
        if (all(filter_shape == 5)) {
            ...
            const uint32_t shared_bytes = (BLOCK_SIZE.x + filter_shape[2] - 1) *
            ...
            stream.enqueue("filter::convolve3", convolve3_<T>, {blocks, BLOCK_SIZE, shared_bytes},
                           input_accessor, output_accessor, uint_shape, int3_t(filter_shape), blocks_x);
        }
        stream.attach(input, output, filter);
    }

    #define NOA_INSTANTIATE_CONV3_(T) \
    template void convolve3<T,T,void>(const shared_t<T[]>&, dim4_t, const shared_t<T[]>&, dim4_t, dim4_t, const shared_t<T[]>&, dim3_t, Stream&)
