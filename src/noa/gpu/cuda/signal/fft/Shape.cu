...
namespace {
    ...
    template<fft::Remap REMAP, typename GeomShape, typename GeomShapeSmooth,
             typename Matrix, typename Value, typename Radius>
    void launch3D_(const shared_t<Value[]>& input, dim4_t input_strides,
                   const shared_t<Value[]>& output, dim4_t output_strides, dim4_t shape,
                   const float3_t& center, const Radius& radius, float edge_size,
                   Matrix inv_matrix, bool invert, cuda::Stream& stream) {
        ...
        if (has_smooth_edge) {
            ...
            if (Matrix{} == inv_matrix) {
                const auto kernel = signal::fft::details::shape3D<REMAP, uint32_t>(
                        input_accessor, output_accessor, shape_3d, geom_shape_smooth, empty_t{});
                cuda::utils::iwise3D("signal::shape", iwise_start, iwise_end, kernel, stream);
            } else {
                const auto kernel = signal::fft::details::shape3D<REMAP, uint32_t>(
                        input_accessor, output_accessor, shape_3d, geom_shape_smooth, inv_matrix);
                cuda::utils::iwise3D("signal::shape", iwise_start, iwise_end, kernel, stream);
            }
        } else {
            ...
            if (Matrix{} == inv_matrix) {
                const auto kernel = signal::fft::details::shape3D<REMAP, uint32_t>(
                        input_accessor, output_accessor, shape_3d, geom_shape, empty_t{});
                cuda::utils::iwise3D("signal::shape", iwise_start, iwise_end, kernel, stream);
            } else {
                const auto kernel = signal::fft::details::shape3D<REMAP, uint32_t>(
                        input_accessor, output_accessor, shape_3d, geom_shape, inv_matrix);
                cuda::utils::iwise3D("signal::shape", iwise_start, iwise_end, kernel, stream);
            }
        }
    }
    ...
    template<fft::Remap REMAP, typename GeomShape, typename GeomShapeSmooth,
            typename Matrix, typename Value, typename Radius>
    void launch2D_(const shared_t<Value[]>& input, dim4_t input_strides,
                   const shared_t<Value[]>& output, dim4_t output_strides, dim4_t shape,
        ...
        if (has_smooth_edge) {
            ...
            if (Matrix{} == inv_matrix) {
                const auto kernel = signal::fft::details::shape2D<REMAP, uint32_t>(
                        input_accessor, output_accessor, shape_2d, geom_shape_smooth, empty_t{});
                cuda::utils::iwise2D("signal::shape", iwise_start, iwise_end, kernel, stream);
            } else {
                const auto kernel = signal::fft::details::shape2D<REMAP, uint32_t>(
                        input_accessor, output_accessor, shape_2d, geom_shape_smooth, inv_matrix);
                cuda::utils::iwise2D("signal::shape", iwise_start, iwise_end, kernel, stream);
            }
        } else {
            const GeomShape geom_shape(center, radius);
            if (Matrix{} == inv_matrix) {
                const auto kernel = signal::fft::details::shape2D<REMAP, uint32_t>(
                        input_accessor, output_accessor, shape_2d, geom_shape, empty_t{});
                cuda::utils::iwise2D("signal::shape", iwise_start, iwise_end, kernel, stream);
            } else {
                const auto kernel = signal::fft::details::shape2D<REMAP, uint32_t>(
                        input_accessor, output_accessor, shape_2d, geom_shape, inv_matrix);
                cuda::utils::iwise2D("signal::shape", iwise_start, iwise_end, kernel, stream);
            }
        }
    }
    ...
    void cylinder(const shared_t<Value[]>& input, dim4_t input_strides,
        ...
        if (invert) {
            ...
            launch3D_<REMAP, cylinder_t, cylinder_smooth_t>(
                    input, input_strides, output, output_strides, shape,
                    center, radius_, edge_size, inv_matrix, invert, stream);
        } else {
