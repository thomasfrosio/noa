...
namespace noa::cuda::signal::fft {
    ...
    void isotropic_fsc(const Complex<Real>* lhs, const Strides4<i64>& lhs_strides,
        ...
        const auto denominator_lhs = noa::cuda::memory::PtrDevice<Real>::alloc(batches * shell_count, stream);
        const auto denominator_rhs = noa::cuda::memory::PtrDevice<Real>::alloc(batches * shell_count, stream);
        ...
        noa::cuda::utils::iwise_4d("isotropic_fsc", s_shape, fsc_reduction, stream);

        using iwise2D_config = noa::cuda::utils::IwiseStaticConfig<noa::cuda::Constant::WARP_SIZE, 1>;
        noa::cuda::utils::iwise_2d<iwise2D_config>(
                "isotropic_fsc", Shape2<i32>{batches, shell_count}, fsc_reduction, stream);
    }
    ...
    void anisotropic_fsc(const Complex<Real>* lhs, const Strides4<i64>& lhs_strides,
        ...
        const auto denominator_lhs = noa::cuda::memory::PtrDevice<Real>::alloc(buffer_size, stream);
        const auto denominator_rhs = noa::cuda::memory::PtrDevice<Real>::alloc(buffer_size, stream);
        ...
        noa::cuda::utils::iwise_4d("anisotropic_fsc", s_shape, fsc_reduction, stream);

        using iwise3D_config = noa::cuda::utils::IwiseStaticConfig<noa::cuda::Constant::WARP_SIZE, 1>;
        noa::cuda::utils::iwise_3d<iwise3D_config>(
                "anisotropic_fsc", Shape3<i32>{batches, cones, shell_count}, fsc_reduction, stream);
    }
