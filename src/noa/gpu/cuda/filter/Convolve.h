...
namespace noa::cuda::filter {
    ...
    /// \param[in] inputs       On the \b device. Array to convolve. One per batch.
    /// \param inputs_pitch     Pitch, in elements, of \p inputs.
    /// \param[out] outputs     On the \b device. Convolved array. One per batch.
    ...
    /// \param[in] filter       On the \b host or \b device. Filter corresponding to the first dimension of \p shape.
    ...
    NOA_HOST void convolve1(const T* inputs, size_t inputs_pitch, T* outputs, size_t outputs_pitch,
                            size3_t shape, size_t batches, const T* filter, size_t filter_size, Stream& stream);
    ...
    /// \param shape            Logical {fast, medium, slow} shape of \p inputs and \p outputs, ignoring the batches.
    /// \param batches          Number of contiguous batches to compute.
    /// \param[in] filter       On the \b host or \b device. 2D filter corresponding to the first two dimensions of \p shape.
    ...
    NOA_HOST void convolve2(const T* inputs, size_t inputs_pitch, T* outputs, size_t outputs_pitch,
                            size3_t shape, size_t batches, const T* filter, size2_t filter_shape, Stream& stream);
    ...
    /// \param shape            Logical {fast, medium, slow} shape of \p inputs and \p outputs, ignoring the batches.
    /// \param batches          Number of contiguous batches to compute.
    /// \param[in] filter       On the \b host or \b device. 3D filter corresponding to \p shape.
    ...
    NOA_HOST void convolve3(const T* inputs, size_t inputs_pitch, T* outputs, size_t outputs_pitch,
                            size3_t shape, size_t batches, const T* filter, size3_t filter_shape, Stream& stream);
    ...
    /// \param shape            Logical {fast, medium, slow} shape of \p inputs and \p outputs, ignoring the batches.
    /// \param batches          Number of contiguous batches to compute.
    /// \param[in] filter       On the \b host or \b device. ND filter corresponding to \p shape.
    /// \param filter_shape     Physical {fast, medium, slow} shape of \p filter.
    ...
    /// \param[in] filter0      On the \b host or \b device. Corresponds to the first dimension of \p shape.
    ...
    /// \param[in] filter1      On the \b host or \b device. Corresponds to the second dimension of \p shape.
    ...
    /// \param[in] filter2      On the \b host or \b device. Corresponds to the third dimension of \p shape.
