...
namespace {
    ...
    void convolve1_(const T* __restrict__ inputs, uint inputs_pitch,
...
namespace noa::cuda::filter {
    ...
    void convolve1(const T* inputs, size_t inputs_pitch, T* outputs, size_t outputs_pitch,
        ...
        convolve1_<<<blocks, threads, shared_bytes, stream.get()>>>(
                inputs, inputs_pitch, outputs, outputs_pitch, int_shape, filter_size, blocks_x);
        NOA_THROW_IF(cudaPeekAtLastError());
    }

    #define NOA_INSTANTIATE_CONV1_(T) \
    template void convolve1<T>(const T*, size_t, T*, size_t, size3_t, size_t, const T*, size_t, Stream&)
