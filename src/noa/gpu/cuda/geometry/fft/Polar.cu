...
namespace {
    ...
    cartesian2polar_(cudaTextureObject_t cartesian, T* output, uint3_t output_stride, uint2_t output_shape,
                     float center, float start_angle, float2_t start_magnitude,
                     float step_angle, float2_t step_magnitude, bool log) {
        ...
        if (log) {
            magnitude_y = math::exp(polar_coordinate[1] * step_magnitude[0]) - 1 + start_magnitude[0];
            magnitude_x = math::exp(polar_coordinate[1] * step_magnitude[1]) - 1 + start_magnitude[1];
        } else {
            magnitude_y = polar_coordinate[1] * step_magnitude[0] + start_magnitude[0];
            magnitude_x = polar_coordinate[1] * step_magnitude[1] + start_magnitude[1];
        }

        // (angle, magnitude) -> (y, x)
        float2_t cartesian_coordinates{magnitude_y * math::sin(angle_rad) + center,
                                       magnitude_x * math::cos(angle_rad)};
        [[maybe_unused]] float conj = 1;
...
namespace noa::cuda::geometry::fft {
    ...
    void cartesian2polar(cudaTextureObject_t cartesian, InterpMode cartesian_interp, size2_t cartesian_shape,
        ...
        float step_magnitude_y, step_magnitude_x;
        if (log) {
            step_magnitude_y = math::log(radius_y_range[1] - radius_y_range[0]) / shape[1];
            step_magnitude_x = math::log(radius_x_range[1] - radius_x_range[0]) / shape[1];
        } else {
            step_magnitude_y = (radius_y_range[1] - radius_y_range[0]) / shape[1];
            step_magnitude_x = (radius_x_range[1] - radius_x_range[0]) / shape[1];
        }
        ...
        switch (cartesian_interp) {
            case INTERP_NEAREST:
                return stream.enqueue("geometry::cartesian2polar",
                                      ...
                                      half_shape[0], angle_range[0], float2_t{radius_y_range[0], radius_x_range[1]},
                                      step_angle, float2_t{step_magnitude_y, step_magnitude_x}, log);
            case INTERP_LINEAR:
                return stream.enqueue("geometry::cartesian2polar",
