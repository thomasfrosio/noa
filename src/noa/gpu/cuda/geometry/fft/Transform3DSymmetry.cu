...
namespace {
    ...
    void transform3D_(cudaTextureObject_t tex, Accessor<T, 4, uint32_t> output,
                      int3_t shape, float3_t norm_shape,
                      [[maybe_unused]] float33_t matrix, const float33_t* sym_matrices, uint32_t sym_count,
                      [[maybe_unused]] float3_t shift, float scaling, float cutoff_sqd, uint32_t blocks_x) {
        ...
        const int32_t w = getFrequency_<IS_DST_CENTERED>(gid[1], shape[0]);
        const int32_t v = getFrequency_<IS_DST_CENTERED>(gid[2], shape[1]);
        ...
        switch (texture_interp_mode) {
            case INTERP_NEAREST:
                return stream.enqueue("geometry::fft::transform3D",
                                      ...
                                      config, texture, output_accessor, s_shape, f_shape,
