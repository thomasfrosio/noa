...
namespace {
    ...
    auto wrap_into_accessor_(
        ...
        if constexpr (!std::is_pointer_v<PointerOrValue>) {
            ...
            return AccessorRestrict<value_t, 3, u32>(pointer_or_value, strides.as_safe<u32>());
        }
    }
}
...
namespace noa::cuda::geometry::fft {
    ...
    void insert_rasterize_3d(
        ...
        if (apply_ews || apply_scale) {
            const auto op = noa::geometry::fourier_insert_rasterize_op<REMAP>(
                    input_slice_accessor, input_weight_accessor, s_input_slice_shape,
                    output_volume_accessor, output_weight_accessor, s_output_volume_shape,
                    inv_scaling, fwd_rotation, fftfreq_cutoff, s_target_shape, ews_radius);
            noa::cuda::utils::iwise_3d(iwise_shape, op, stream);
        } else {
            const auto op = noa::geometry::fourier_insert_rasterize_op<REMAP>(
                    input_slice_accessor, input_weight_accessor, s_input_slice_shape,
                    output_volume_accessor, output_weight_accessor, s_output_volume_shape,
                    Empty{}, fwd_rotation, fftfreq_cutoff, s_target_shape, Empty{});
            noa::cuda::utils::iwise_3d(iwise_shape, op, stream);
        }
    }
    ...
    template void insert_rasterize_3d<REMAP, IValue, IWeight, OValue, OWeight, Scale, Rotate>(  \
        ...
        Scale const&, Rotate const&, f32, const Shape4<i64>&, const Vec2<f32>&, Stream&)
