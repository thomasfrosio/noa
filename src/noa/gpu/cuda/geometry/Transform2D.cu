...
namespace {
    ...
    transform2D_(interpolator_t interpolator,
        ...
        if constexpr (traits::is_any_v<matrix_t, const float23_t*, const float33_t*>) {
            ...
            const float2_t coordinates = matrices * pos;
            output(gid) = interpolator(coordinates);
        } else {
        ...
        if (texture_border_mode == BORDER_PERIODIC || texture_border_mode == BORDER_MIRROR) {
            ...
            if (texture_interp_mode == INTERP_NEAREST) {
                ...
                stream.enqueue("geometry::transform2D",
                               transform2D_<data_t, interpolator_t, matrix_t>, config,
                               interpolator_t(texture, i_shape), output_accessor, o_shape, matrices);
            ...
            switch (texture_interp_mode) {
                case INTERP_NEAREST: {
                    ...
                    return stream.enqueue(
                            ...
                            transform2D_<data_t, interpolator_t, matrix_t>, config,
                            interpolator_t(texture), output_accessor, o_shape, matrices);
                }
                case INTERP_LINEAR: {
