...
namespace {
    ...
    transformWithSymmetry2D_(cudaTextureObject_t texture, Accessor<T, 3, uint32_t> output, uint2_t shape,
                             float2_t shift, float22_t matrix, float2_t center,
                             const float33_t* symmetry_matrices, uint32_t symmetry_count, float scaling) {
        ...
        switch (texture_interp_mode) {
            case INTERP_NEAREST:
                return stream.enqueue("geometry::transform2D", transformWithSymmetry2D_<T, INTERP_NEAREST>,
                                      config, texture, output_accessor, o_shape, shift, matrix, center,
