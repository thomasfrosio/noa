...
namespace {
    ...
    transform3D_(cudaTextureObject_t texture, float3_t texture_shape,
        ...
        output[indexing::at(gid, output_stride)] = cuda::geometry::tex3D<T, MODE>(texture, coordinates);
    }
    ...
    void launchTransform3D_(const shared_t<T[]>& input, size4_t input_stride, size4_t input_shape,
