...
namespace {
    ...
    void launchTransformTexture3D_(cudaTextureObject_t texture, size3_t texture_shape,
        ...
        if (texture_border_mode == BORDER_PERIODIC || texture_border_mode == BORDER_MIRROR) {
            ...
            if (texture_interp_mode == INTERP_NEAREST) {
                stream.enqueue("geometry::transform3D",
                               ...
                               config, texture, i_shape, output, o_strides, o_shape, matrices, blocks_x);
    ...
    template<typename T, typename MAT>
    void launchTransformTextureSingle3D_(cudaTextureObject_t texture, size3_t texture_shape,
                                         InterpMode texture_interp_mode, BorderMode texture_border_mode,
                                         T* output, size4_t output_strides, size4_t output_shape,
        ...
        if (texture_border_mode == BORDER_PERIODIC || texture_border_mode == BORDER_MIRROR) {
            ...
            if (texture_interp_mode == INTERP_NEAREST) {
                stream.enqueue("geometry::transform3D",
                               ...
                               config, texture, i_shape, output, o_strides, o_shape, matrix_, blocks_x);
            } else if (texture_interp_mode == INTERP_LINEAR_FAST) {
                stream.enqueue("geometry::transform3D",
