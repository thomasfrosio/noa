...
namespace noa::cuda::guts {
    ...
    void copy(
        ...
        // ewise(
        //      Copy{}, shape,
        //      make_tuple(Accessor<const T, 4, u32>(src, src_strides),
        //      make_tuple(Accessor<T, 4, u32>(dst, dst_strides),
        //      stream);
        panic("TODO");
    }
}
...
namespace noa::cuda {
    ...
    inline void copy(const T* src, T* dst, i64 elements) {
        ...
        check(cudaMemcpy(dst, src, count, cudaMemcpyDefault));
    }
    ...
    void copy(
        ...
        for (i32 test = 0; test <= 1; ++test) {
            ...
            is_contiguous = ni::is_contiguous(src_strides, shape) &&
            ...
            if (test == 0) { // try once
                ...
                if (ni::reshape(shape, src_strides, collapsed_shape, new_src_strides) &&
        ...
        const cudaPointerAttributes src_attr = noa::cuda::pointer_attributes(src);
        const cudaPointerAttributes dst_attr = noa::cuda::pointer_attributes(dst);
        ...
        if (src_attr.type == 2 && dst_attr.type == 2) {
            ...
            if (src_attr.device == dst_attr.device) {
                guts::copy(src, src_strides, dst, dst_strides, shape, stream);
            } else {
            ...
            guts::copy(reinterpret_cast<const T*>(src_attr.devicePointer), src_strides,
            ...
            NOA_ASSERT(!ni::are_overlapped(src, src_strides, dst, dst_strides, shape));
            ...
            panic("Copying strided regions, other than in the height dimension, between an unregistered "
...
namespace noa::cuda {
    ...
    void copy(
        ...
        check(expected_extent.depth == actual_extent.depth &&
        ...
        check(is_rightmost && has_valid_pitch && is_contiguous_0 && is_contiguous_2,
