...
namespace {
    ...
    void transpose102_(const T* __restrict__ inputs, uint inputs_pitch,
    ...
    template<typename T, bool IS_MULTIPLE_OF_TILE>
    __global__ __launch_bounds__(THREADS.x * THREADS.y)
    void transpose102_(T* outputs, uint outputs_pitch, uint shapeXY, uint shapeZ, uint blocks_x) {
...
template void noa::cuda::memory::details::transpose102<T>(const T*, size_t, T*, size_t, size3_t, size_t, Stream&);  \
template void noa::cuda::memory::details::inplace::transpose102<T>(T*, size_t, size3_t, size_t, Stream&)
