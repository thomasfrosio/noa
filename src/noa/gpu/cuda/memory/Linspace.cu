...
namespace {
    ...
    template<typename T>
    __global__ __launch_bounds__(BLOCK_SIZE)
    void linspace4D_(T* src, uint4_t strides, uint4_t shape,
        ...
        for (int k = 0; k < ELEMENTS_PER_THREAD_2D.y; ++k) {
            ...
            for (int l = 0; l < ELEMENTS_PER_THREAD_2D.x; ++l) {
                ...
                const uint offset = ik * strides[2] + il * strides[3];
                if (endpoint && is_last_ij && ik == last[2] && il == last[3])
                    src[offset] = stop;
                else if (ik < shape[2] && il < shape[3])
                    src[offset] = start + static_cast<T>(indexing::at(gid[0], gid[1], ik, il, logical_strides)) * step;
            }
        }
    }
}
...
namespace noa::cuda::memory {
    ...
    T linspace(const shared_t<T[]>& src, size4_t strides, size4_t shape,
        ...
        if (is_contiguous[0] && is_contiguous[1] && is_contiguous[2]) {
            ...
            stream.enqueue("memory::linspace", linspace4D_<T>, {blocks, BLOCK_SIZE_2D},
                           src.get(), uint4_t(strides), uint_shape, start, stop, step, endpoint, blocks_x);
        }
        stream.attach(src);
        return step;
    }
    ...
    template T linspace<T, void>(const shared_t<T[]>&, size_t, T, T, bool, Stream&); \
    template T linspace<T, void>(const shared_t<T[]>&, size4_t, size4_t, T, T, bool, Stream&)
