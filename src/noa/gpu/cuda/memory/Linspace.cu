...
namespace noa::cuda::memory {
    ...
    T linspace(const Shared<T[]>& src, i64 elements,
        ...
        const auto [kernel, step] = noa::algorithm::memory::linspace_1d<i64, i64>(
                src.get(), elements, start, stop, endpoint);
    ...
    T linspace(const Shared<T[]>& src, const Strides4<i64>& strides, const Shape4<i64>& shape,
        ...
        const auto [kernel, step] = noa::algorithm::memory::linspace_4d<i64, i64>(
                src.get(), strides, shape, start, stop, endpoint);
    ...
    template T linspace<T, void>(const Shared<T[]>&, i64, T, T, bool, Stream&); \
    template T linspace<T, void>(const Shared<T[]>&, const Strides4<i64>&, const Shape4<i64>&, T, T, bool, Stream&)
