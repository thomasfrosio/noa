...
namespace noa::cuda::memory::details {
    ...
    template<typename T>
    void permute_0321_inplace(const Shared<T[]>& output, const Strides4<i64>& output_strides,
                              const Shape4<i64>& shape, Stream& stream) {
        NOA_ASSERT_DEVICE_PTR(output.get(), stream.device());
        if (shape[1] != shape[3])
        ...
        const auto output_accessor = Accessor<T, 4, u32>(output.get(), output_strides.as_safe<u32>());
        const auto swapped_output = output_accessor.swap_dimensions(1, 2);
