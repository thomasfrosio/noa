...
namespace {
    ...
    void cropH2H_(const T* __restrict__ input, uint4_t input_strides, uint3_t input_shape,
                  T* __restrict__ output, uint4_t output_strides, uint3_t output_shape, uint blocks_x) {
        ...
        input += indexing::at(gid[0], iz, iy, input_strides);
    ...
    template<class T>
    __global__ __launch_bounds__(BLOCK_SIZE.x * BLOCK_SIZE.y)
    void cropF2F_(const T* __restrict__ input, uint4_t input_strides, uint3_t input_shape,
                  T* __restrict__ output, uint4_t output_strides, uint3_t output_shape, uint blocks_x) {
        const uint2_t index = indexing::indexes(blockIdx.x, blocks_x);
        const uint4_t gid{blockIdx.z,
                          blockIdx.y,
        ...
        for (int i = 0; i < ELEMENTS_PER_THREAD_X; ++i) {
            ...
            if (ox < output_shape[2])
                output[ox * output_strides[3]] = input[ix * input_strides[3]];
        }
    }

    template<class T>
    __global__ __launch_bounds__(BLOCK_SIZE.x * BLOCK_SIZE.y)
    void padH2H_(const T* __restrict__ input, uint4_t input_strides, uint3_t input_shape,
                 T* __restrict__ output, uint4_t output_strides, uint3_t output_shape, uint blocks_x) {
        const uint2_t index = indexing::indexes(blockIdx.x, blocks_x);
        const uint4_t gid{blockIdx.z,
                          blockIdx.y,
        ...
        output += indexing::at(gid[0], oz, oy, output_strides);
    ...
    template<class T>
    __global__ __launch_bounds__(BLOCK_SIZE.x * BLOCK_SIZE.y)
    void padF2F_(const T* __restrict__ input, uint4_t input_strides, uint3_t input_shape,
                 T* __restrict__ output, uint4_t output_strides, uint3_t output_shape, uint blocks_x) {
        const uint2_t index = indexing::indexes(blockIdx.x, blocks_x);
        const uint4_t gid{blockIdx.z,
                          blockIdx.y,
...
namespace noa::cuda::fft::details {
    ...
    void cropH2H(const shared_t<T[]>& input, size4_t input_strides, size4_t input_shape,
        ...
        stream.enqueue("cropH2H_", cropH2H_<T>, {blocks, BLOCK_SIZE},
                       input.get(), uint4_t(input_strides), old_shape,
                       output.get(), uint4_t(output_strides), new_shape, blocks_x);
