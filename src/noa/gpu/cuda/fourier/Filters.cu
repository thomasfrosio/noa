...
namespace {
    ...
    NOA_ID float getDistanceSquared(size_t dimension, uint half, size_t idx) {
...
namespace {
    ...
    template<Type PASS>
    NOA_ID float getSoftWindow(float freq_cutoff, float freq_width, float freq) {
    ...
    __global__ void singlePassSoft(T* inputs, uint pitch_inputs, T* outputs, uint pitch_outputs,
        ...
        for (uint x = threadIdx.x; x < half.x; x += blockDim.x) {
            ...
            filter = static_cast<real_t>(getSoftWindow<PASS>(freq_cutoff, freq_width, frequency));
    ...
    template<Type PASS, typename T>
    __global__ void singlePassSoft(T* output_filter, uint pitch_output_filter, uint3_t shape, uint3_t half,
    ...
    template<typename T>
    __global__ void bandPassSoft(T* inputs, uint pitch_inputs, T* outputs, uint pitch_outputs,
    ...
    template<typename T>
    __global__ void bandPassSoft(T* output_filter, uint pitch_output_filter, uint3_t shape, uint3_t half,
...
namespace {
    ...
    template<Type PASS>
    NOA_ID float getHardWindow(float freq_cutoff_sqd, float freq_sqd) {
        float filter;
        if constexpr (PASS == Type::LOWPASS) {
    ...
    template<Type PASS, typename T>
    __global__ void singlePassHard(T* inputs, uint pitch_inputs, T* outputs, uint pitch_outputs,
        ...
        for (uint x = threadIdx.x; x < half.x; x += blockDim.x) {
            ...
            filter = static_cast<real_t>(getHardWindow<PASS>(freq_cutoff_sqd, frequency_sqd));
            for (uint batch = 0; batch < batches; ++batch)
                outputs[batch * elements_outputs + x] = inputs[batch * elements_inputs + x] * filter;
        }
    }
    ...
    template<Type PASS, typename T>
    __global__ void singlePassHard(T* output_filter, uint pitch_output_filter,
        ...
        for (uint x = threadIdx.x; x < half.x; x += blockDim.x) {
            ...
            distance_sqd.x *= distance_sqd.x;
            frequency_sqd = Math::sum(distance_sqd); // from 0 to 0.25
            filter = getHardWindow<PASS>(freq_cutoff_sqd, frequency_sqd);
            output_filter[x] = static_cast<real_t>(filter);
        }
    }

    template<typename T>
    __global__ void bandPassHard(T* inputs, uint pitch_inputs, T* outputs, uint pitch_outputs,
        ...
        for (uint x = threadIdx.x; x < half.x; x += blockDim.x) {
            ...
            distance_sqd.x *= distance_sqd.x;
            frequency_sqd = Math::sum(distance_sqd); // from 0 to 0.25
            filter = getHardWindow<Type::HIGHPASS>(freq_cutoff_sqd_1, frequency_sqd);
            filter *= getHardWindow<Type::LOWPASS>(freq_cutoff_sqd_2, frequency_sqd);
            for (uint batch = 0; batch < batches; ++batch)
                outputs[batch * elements_outputs + x] =
                        inputs[batch * elements_inputs + x] * static_cast<real_t>(filter);
        }
    ...
    template<typename T>
    __global__ void bandPassHard(T* output_filter, uint pitch_output_filter, uint3_t shape, uint3_t half,
...
namespace Noa::CUDA::Fourier {
    ...
    void highpass(T* inputs, size_t pitch_inputs, T* outputs, size_t pitch_outputs, size3_t shape,
        ...
        if (freq_width > 1e-8f) {
            NOA_CUDA_LAUNCH(blocks, threads, 0, stream.id(),
                            singlePassSoft<Type::HIGHPASS>,
                            inputs, pitch_inputs, outputs, pitch_outputs, tmp_shape, half,
                            freq_cutoff, freq_width, batches);
        } else {
            NOA_CUDA_LAUNCH(blocks, threads, 0, stream.id(),
                            singlePassHard<Type::HIGHPASS>,
                            inputs, pitch_inputs, outputs, pitch_outputs, tmp_shape, half,
                            freq_cutoff, batches);
        }
    }
