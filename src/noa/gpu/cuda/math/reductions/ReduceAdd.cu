...
namespace Noa::CUDA::Math::Details::Contiguous {
    ...
    template<typename T>
    static __global__ void reduceMean(T* inputs, T* outputs, uint elements, int vectors);
...
namespace Noa::CUDA::Math {
    ...
    void reduceMeanWeighted(T* inputs, size_t pitch_inputs,
        ...
        NOA_CUDA_LAUNCH(blocks, threads, 0, stream.id(),
                        Details::Padded::reduceMeanWeighted,
...
namespace Noa::CUDA::Math::Details::Contiguous {
    ...
    template<typename T>
    __global__ void reduceMean(T* inputs, T* outputs, uint elements, int vectors) {
        inputs += blockIdx.y * vectors * elements;
        outputs += blockIdx.y * elements;
