...
namespace {
    ...
    namespace contiguous_ {
        ...
        void getLaunchConfig_(uint elements, uint* output_blocks, uint* output_threads) {
            ...
            *output_blocks = noa::math::divideUp(elements, *output_threads * 2);
            *output_blocks = noa::math::min(*output_blocks, MAX_BLOCKS);
        }

        template<typename T>
        __global__ __launch_bounds__(MAX_THREADS)
        void reduceAdd_(const T* inputs, T* outputs, uint elements, int vectors) {
        ...
        template<typename T>
        __global__ __launch_bounds__(MAX_THREADS)
        void reduceMean_(const T* inputs, T* outputs, uint elements, int vectors) {
            inputs += blockIdx.y * vectors * elements;
            outputs += blockIdx.y * elements;
        ...
        __global__ __launch_bounds__(MAX_THREADS)
        void reduceMeanWeighted_(const T* inputs, const U* weights, T* outputs, uint elements, int vectors) {
            inputs += blockIdx.y * vectors * elements;
            outputs += blockIdx.y * elements;
    ...
    namespace padded_ {
        ...
        void getLaunchConfig_(uint2_t shape_2d, uint batches, dim3* output_blocks, dim3* output_threads) {
            ...
            output_blocks->x = noa::math::min(noa::math::divideUp(shape_2d.y, WARPS), MAX_BLOCKS);
        ...
        void reduceAdd_(const T* inputs, uint inputs_pitch, T* outputs, uint outputs_pitch,
        ...
        template<typename T>
        __global__ __launch_bounds__(BLOCK_SIZE.x * BLOCK_SIZE.y)
        void reduceMean_(const T* inputs, uint inputs_pitch, T* outputs, uint outputs_pitch,
                         int vectors, uint2_t shape) {
            uint size_input = inputs_pitch * shape.y;
            inputs += blockIdx.y * vectors * size_input;
            outputs += blockIdx.y * outputs_pitch * shape.y;
        ...
        template<typename T, typename U>
        __global__ __launch_bounds__(BLOCK_SIZE.x * BLOCK_SIZE.y)
        void reduceMeanWeighted_(const T* inputs, uint inputs_pitch,
...
namespace noa::cuda::math {
    template<typename T>
    void reduceAdd(const T* inputs, T* outputs, size_t elements, size_t vectors, size_t batches, Stream& stream) {
    ...
    template<typename T>
    void reduceMean(const T* inputs, T* outputs, size_t elements, size_t nb_to_reduce, size_t batches, Stream& stream) {
        NOA_PROFILE_FUNCTION();
        uint blocks, threads;
        contiguous_::getLaunchConfig_(elements, &blocks, &threads);
    ...
    template void reduceAdd<T>(const T*, T*, size_t, size_t, size_t, Stream&);                  \
    template void reduceAdd<T>(const T*, size_t, T*, size_t, size3_t, size_t, size_t, Stream&); \
    template void reduceMean<T>(const T*, T*, size_t, size_t, size_t, Stream&);                 \
    template void reduceMean<T>(const T*, size_t, T*, size_t, size3_t, size_t, size_t, Stream&)
    ...
    template void reduceMeanWeighted<T, U>(const T*, const U*, T*, size_t, size_t, size_t, Stream&);    \
    template void reduceMeanWeighted<T, U>(const T*, size_t, const U*, size_t, T*, size_t, size3_t, size_t, size_t, Stream&)
