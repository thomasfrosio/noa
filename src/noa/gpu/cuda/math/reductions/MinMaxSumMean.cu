...
namespace Noa::CUDA::Math::Details::Contiguous {
    ...
    template<typename T>
    static void launch(T* input, T* tmp_mins, T* tmp_maxs, T* tmp_sums,
                       uint elements, uint blocks, cudaStream_t stream);

    template<bool TWO_BY_TWO, typename T>
    static __global__ void kernel(T* input, T* tmp_mins, T* tmp_maxs, T* tmp_sums, uint elements);
}
...
namespace Noa::CUDA::Math::Details::Padded {
    ...
    template<typename T>
    static void launch(T* input, uint pitch, T* tmp_mins, T* tmp_maxs, T* tmp_sums,
                       uint2_t shape, uint blocks, cudaStream_t stream);

    template<bool TWO_BY_TWO, typename T>
    static __global__ void kernel(T* input, uint pitch, T* tmp_mins, T* tmp_maxs, T* tmp_sums, uint2_t shape);
}
...
namespace Noa::CUDA::Math::Details::Final {
    ...
    template<typename T>
    static void launch(T* inputs, T* output_mins, T* output_maxs, T* output_sums, T* output_means,
    ...
    // Kernel to reduce the intermediary results (3 input arrays, per block). Computes 4 (2 optional) values per batch.
    template<typename T>
    static void launch(T* tmp_mins, T* tmp_maxs, T* tmp_sums,
                       T* output_mins, T* output_maxs, T* output_sums, T* output_means,
                       size_t elements, T scale, uint batches, uint threads, cudaStream_t stream);

    template<int BLOCK_SIZE, bool TWO_BY_TWO, typename T>
    static __global__ void kernel(T* tmp_mins, T* tmp_maxs, T* tmp_sums, uint elements,
                                  T* output_mins, T* output_maxs, T* output_sums, T* output_means, T scale);
}
        ...
        if (elements <= 32768 || batches > 16) {
            if (elements) {
                ...
                for (int batch = 0; batch < batches; batch += 32768U) {
                    ...
                    Details::Final::launch(input, mins, maxs, sums, means,
...
namespace Noa::CUDA::Math::Details {
    ...
    template<typename T>
    static NOA_ID void inPlaceMinMaxSum(T* current_min, T* current_max, T* current_sum, T candidate) {
    ...
    template<typename T>
    static NOA_ID void reduceSharedData(int tid, T* s_mins, T* s_maxs, T* s_sums,
...
namespace Noa::CUDA::Math::Details::Contiguous {
    ...
    template<typename T>
    void launch(T* input, T* tmp_mins, T* tmp_maxs, T* tmp_sums,
                uint elements, uint blocks, cudaStream_t stream) {
        bool two_by_two = !(elements % (BLOCK_SIZE * 2));
        if (two_by_two) {
            kernel<true><<<blocks, BLOCK_SIZE, 0, stream>>>(input, tmp_mins, tmp_maxs, tmp_sums, elements);
        } else {
            kernel<false><<<blocks, BLOCK_SIZE, 0, stream>>>(input, tmp_mins, tmp_maxs, tmp_sums, elements);
        }
...
namespace Noa::CUDA::Math::Details::Padded {
    ...
    template<typename T>
    void launch(T* input, uint pitch, T* tmp_mins, T* tmp_maxs, T* tmp_sums,
                uint2_t shape, uint blocks, cudaStream_t stream) {
        dim3 threads(BLOCK_SIZE.x, BLOCK_SIZE.y);
        bool two_by_two = !(shape.x % (BLOCK_SIZE.x * 2));
        if (two_by_two) {
            kernel<true><<<blocks, threads, 0, stream>>>(input, pitch, tmp_mins, tmp_maxs, tmp_sums, shape);
        } else {
            kernel<false><<<blocks, threads, 0, stream>>>(input, pitch, tmp_mins, tmp_maxs, tmp_sums, shape);
        }
        NOA_THROW_IF(cudaPeekAtLastError());
    }
}
...
namespace Noa::CUDA::Math::Details::Final {
    ...
    template<typename T>
    void launch(T* inputs, T* output_mins, T* output_maxs, T* output_sums, T* output_means,
                size_t elements, T scale, uint batches, uint threads, cudaStream_t stream) {
        bool two_by_two = !(elements % (threads * 2));
        if (two_by_two) {
            switch (threads) {
                case 256:
                    kernel<256, true><<<batches, 256, 0, stream>>>(inputs, elements,
                                                                   output_mins, output_maxs,
                                                                   output_sums, output_means, scale);
                    break;
                case 128:
                    kernel<128, true><<<batches, 128, 0, stream>>>(inputs, elements,
                                                                   output_mins, output_maxs,
                                                                   output_sums, output_means, scale);
                    break;
                case 64:
                    kernel<64, true><<<batches, 64, 0, stream>>>(inputs, elements,
                                                                 output_mins, output_maxs,
                                                                 output_sums, output_means, scale);
                    break;
                case 32:
                    kernel<32, true><<<batches, 32, 0, stream>>>(inputs, elements,
                                                                 output_mins, output_maxs,
                                                                 output_sums, output_means, scale);
                    break;
                default:
            ...
            switch (threads) {
                case 256:
                    kernel<256, false><<<batches, 256, 0, stream>>>(inputs, elements,
                                                                    output_mins, output_maxs,
                                                                    output_sums, output_means, scale);
                    break;
                case 128:
                    kernel<128, false><<<batches, 128, 0, stream>>>(inputs, elements,
                                                                    output_mins, output_maxs,
                                                                    output_sums, output_means, scale);
                    break;
                case 64:
                    kernel<64, false><<<batches, 64, 0, stream>>>(inputs, elements,
                                                                  output_mins, output_maxs,
                                                                  output_sums, output_means, scale);
                    break;
                case 32:
                    kernel<32, false><<<batches, 32, 0, stream>>>(inputs, elements,
                                                                  output_mins, output_maxs,
                                                                  output_sums, output_means, scale);
                    break;
                default:
