...
namespace {
    ...
    namespace Contiguous_ {
        ...
        __global__ void sum_(T* input, T* mean, T* tmp_sums, uint elements) {
        ...
        template<typename T>
        void launch_(T* input, T* mean, T* tmp_sums, uint elements, uint blocks, cudaStream_t stream) {
    ...
    namespace Padded_ {
         ...
         */
        template<bool TWO_BY_TWO, typename T>
        __global__ void sum_(T* input, uint pitch, T* mean, T* tmp_sums, uint2_t shape) {
        ...
        template<typename T>
        void launch_(T* input, uint pitch, T* mean, T* tmp_sums, uint2_t shape, uint blocks, cudaStream_t stream) {
    ...
    namespace Final_ {
        ...
        __global__ void sumVarStddev_(T* inputs, uint elements, T* means, T scale,
        ...
        template<typename T>
        void launch_(T* input, size_t elements, T* means, T scale, T* output_variances, T* output_stddevs,
                     uint batches, uint threads, cudaStream_t stream) {
            bool two_by_two = !(elements % (threads * 2));
            if (two_by_two) {
                switch (threads) {
                    case 256:
                        sumVarStddev_<256, true><<<batches, 256, 0, stream>>>(input, elements, means, scale,
                                                                              output_variances, output_stddevs);
                        break;
                    case 128:
                        sumVarStddev_<128, true><<<batches, 128, 0, stream>>>(input, elements, means, scale,
                                                                              output_variances, output_stddevs);
                        break;
                    case 64:
                        sumVarStddev_<64, true><<<batches, 64, 0, stream>>>(input, elements, means, scale,
                                                                            output_variances, output_stddevs);
                        break;
                    case 32:
                        sumVarStddev_<32, true><<<batches, 32, 0, stream>>>(input, elements, means, scale,
                                                                            output_variances, output_stddevs);
                        break;
                    default:
                ...
                switch (threads) {
                    case 256:
                        sumVarStddev_<256, false><<<batches, 256, 0, stream>>>(input, elements, means, scale,
                                                                               output_variances, output_stddevs);
                        break;
                    case 128:
                        sumVarStddev_<128, false><<<batches, 128, 0, stream>>>(input, elements, means, scale,
                                                                               output_variances, output_stddevs);
                        break;
                    case 64:
                        sumVarStddev_<64, false><<<batches, 64, 0, stream>>>(input, elements, means, scale,
                                                                             output_variances, output_stddevs);
                        break;
                    case 32:
                        sumVarStddev_<32, false><<<batches, 32, 0, stream>>>(input, elements, means, scale,
                                                                             output_variances, output_stddevs);
                        break;
                    default:
                        NOA_THROW("DEV: block size should be 32, 64, 128 or 256, "
         ...
         */
        template<int BLOCK_SIZE, bool TWO_BY_TWO, typename T>
        __global__ void sumVarStddev_(T* tmp_sums, uint elements, T scale, T* output_variances, T* output_stddevs) {
            static_assert(BLOCK_SIZE >= 32 && BLOCK_SIZE <= 256);
            __shared__ T s_data[BLOCK_SIZE];
        ...
        template<typename T>
        void launch_(T* tmp_sums, size_t elements, T scale, T* output_variances, T* output_stddevs,
                     uint batches, uint threads, cudaStream_t stream) {
            bool two_by_two = !(elements % (threads * 2));
            if (two_by_two) {
                switch (threads) {
                    case 256:
                        sumVarStddev_<256, true><<<batches, threads, 0, stream>>>(tmp_sums, elements, scale,
                                                                                  output_variances, output_stddevs);
                        break;
                    case 128:
                        sumVarStddev_<128, true><<<batches, threads, 0, stream>>>(tmp_sums, elements, scale,
                                                                                  output_variances, output_stddevs);
                        break;
                    case 64:
                        sumVarStddev_<64, true><<<batches, threads, 0, stream>>>(tmp_sums, elements, scale,
                                                                                 output_variances, output_stddevs);
                        break;
                    case 32:
                        sumVarStddev_<32, true><<<batches, threads, 0, stream>>>(tmp_sums, elements, scale,
                                                                                 output_variances, output_stddevs);
                        break;
                    default:
                        NOA_THROW("DEV: block size should be 32, 64, 128 or 256, "
                ...
                switch (threads) {
                    case 256:
                        sumVarStddev_<256, false><<<batches, threads, 0, stream>>>(tmp_sums, elements, scale,
                                                                                   output_variances, output_stddevs);
                        break;
                    case 128:
                        sumVarStddev_<128, false><<<batches, threads, 0, stream>>>(tmp_sums, elements, scale,
                                                                                   output_variances, output_stddevs);
                        break;
                    case 64:
                        sumVarStddev_<64, false><<<batches, threads, 0, stream>>>(tmp_sums, elements, scale,
                                                                                  output_variances, output_stddevs);
                        break;
                    case 32:
                        sumVarStddev_<32, false><<<batches, threads, 0, stream>>>(tmp_sums, elements, scale,
                                                                                  output_variances, output_stddevs);
                        break;
                    default:
                        NOA_THROW("DEV: block size should be 32, 64, 128 or 256, "
...
namespace Noa::CUDA::Math {
    ...
    void varianceStddev(T* inputs, size_t pitch_inputs, T* means, T* output_variances, T* output_stddevs,
        ...
        for (uint batch = 0; batch < batches; ++batch) {
            ...
            T* tmp_sums = tmp.get() + batch * blocks;
            Padded_::launch_(input, pitch_inputs, means + batch, tmp_sums, shape_2d, blocks, stream.get());
        }
        uint threads = Final_::getThreads_(blocks);
        auto scale = static_cast<T>(elements);
        Final_::launch_(tmp.get(), blocks, scale, output_variances, output_stddevs,
