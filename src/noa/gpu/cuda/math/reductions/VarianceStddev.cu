...
namespace Noa::CUDA::Math::Details::Final {
    ...
    template<typename T>
    static void launch(T* input, size_t elements, T* means, T scale, T* output_variances, T* output_stddevs,
                       uint batches, uint threads, cudaStream_t stream);

    template<int BLOCK_SIZE, bool TWO_BY_TWO, typename T>
    static __global__ void kernel(T* inputs, uint elements, T* means, T scale, T* output_variances, T* output_stddevs);
    ...
    template<typename T>
    static void launch(T* tmp_sums, size_t elements, T scale, T* output_variances, T* output_stddevs,
                       uint batches, uint threads, cudaStream_t stream);

    template<int BLOCK_SIZE, bool TWO_BY_TWO, typename T>
    static __global__ void kernel(T* tmp_sums, uint elements, T scale, T* output_variances, T* output_stddevs);
}
...
namespace Noa::CUDA::Math {
    ...
    void varianceStddev(T* inputs, T* means, T* output_variances, T* output_stddevs,
        ...
        if (elements <= 4096 || batches > 16) {
            ...
            auto scale = static_cast<T>(elements);
            Details::Final::launch(tmp.get(), blocks, scale, output_variances, output_stddevs,
    ...
    template<typename T>
    void sumMeanVarianceStddev(T* inputs, size_t pitch_inputs,
                               T* output_sums, T* output_means, T* output_variances, T* output_stddevs,
                               size3_t shape, uint batches, Stream& stream) {
        if (output_means) {
    ...
    template<typename T>
    void statistics(T* inputs, T* output_mins, T* output_maxs, T* output_sums,
    ...
    template<typename T>
    void statistics(T* inputs, size_t pitch_inputs,
...
namespace Noa::CUDA::Math::Details {
     ...
     */
    template<typename T>
    static NOA_DEVICE void sumReduceSharedMemory(int tid, T* s_data, T* tmp_sum) {
        if (tid < 256)
    ...
    static NOA_DEVICE void sumReduceSharedMemory(int tid, T* s_data, T scale, T* output_variance, T* output_stddev) {
...
namespace Noa::CUDA::Math::Details::Contiguous {
    ...
    void launch(T* input, T mean, T* tmp_sums, uint elements, uint blocks, cudaStream_t stream) {
        ...
        if (two_by_two) {
            ...
            kernel<false><<<blocks, BLOCK_SIZE, 0, stream>>>(input, mean, tmp_sums, elements);
        }
...
namespace Noa::CUDA::Math::Details::Padded {
    ...
    void launch(T* input, uint pitch, T mean, T* tmp_sums, uint2_t shape, uint blocks, cudaStream_t stream) {
        ...
        if (two_by_two) {
            kernel<true><<<blocks, threads, 0, stream>>>(input, pitch, mean, tmp_sums, shape);
        } else {
            kernel<false><<<blocks, threads, 0, stream>>>(input, pitch, mean, tmp_sums, shape);
        }
        NOA_THROW_IF(cudaPeekAtLastError());
    }
}
...
namespace Noa::CUDA::Math::Details::Final {
     ...
     */
    template<int BLOCK_SIZE, bool TWO_BY_TWO, typename T>
    __global__ void kernel(T* inputs, uint elements, T* means, T scale,
    ...
    template<typename T>
    void launch(T* input, size_t elements, T* means, T scale, T* output_variances, T* output_stddevs,
                uint batches, uint threads, cudaStream_t stream) {
        bool two_by_two = !(elements % (threads * 2));
        if (two_by_two) {
            switch (threads) {
                case 256:
                    kernel<256, true><<<batches, 256, 0, stream>>>(input, elements, means, scale,
                                                                   output_variances, output_stddevs);
                    break;
                case 128:
                    kernel<128, true><<<batches, 128, 0, stream>>>(input, elements, means, scale,
                                                                   output_variances, output_stddevs);
                    break;
                case 64:
                    kernel<64, true><<<batches, 64, 0, stream>>>(input, elements, means, scale,
                                                                 output_variances, output_stddevs);
                    break;
                case 32:
                    kernel<32, true><<<batches, 32, 0, stream>>>(input, elements, means, scale,
                                                                 output_variances, output_stddevs);
                    break;
                default:
            ...
            switch (threads) {
                case 256:
                    kernel<256, false><<<batches, 256, 0, stream>>>(input, elements, means, scale,
                                                                    output_variances, output_stddevs);
                    break;
                case 128:
                    kernel<128, false><<<batches, 128, 0, stream>>>(input, elements, means, scale,
                                                                    output_variances, output_stddevs);
                    break;
                case 64:
                    kernel<64, false><<<batches, 64, 0, stream>>>(input, elements, means, scale,
                                                                  output_variances, output_stddevs);
                    break;
                case 32:
                    kernel<32, false><<<batches, 32, 0, stream>>>(input, elements, means, scale,
                                                                  output_variances, output_stddevs);
                    break;
                default:
                    NOA_THROW("DEV: block size should be 32, 64, 128 or 256, "
     ...
     */
    template<int BLOCK_SIZE, bool TWO_BY_TWO, typename T>
    __global__ void kernel(T* tmp_sums, uint elements, T scale, T* output_variances, T* output_stddevs) {
        static_assert(BLOCK_SIZE >= 32 && BLOCK_SIZE <= 256);
        __shared__ T s_data[BLOCK_SIZE];
    ...
    template<typename T>
    void launch(T* tmp_sums, size_t elements, T scale, T* output_variances, T* output_stddevs,
                uint batches, uint threads, cudaStream_t stream) {
        bool two_by_two = !(elements % (threads * 2));
        if (two_by_two) {
            switch (threads) {
                case 256:
                    kernel<256, true><<<batches, threads, 0, stream>>>(tmp_sums, elements, scale,
                                                                       output_variances, output_stddevs);
                    break;
                case 128:
                    kernel<128, true><<<batches, threads, 0, stream>>>(tmp_sums, elements, scale,
                                                                       output_variances, output_stddevs);
                    break;
                case 64:
                    kernel<64, true><<<batches, threads, 0, stream>>>(tmp_sums, elements, scale,
                                                                      output_variances, output_stddevs);
                    break;
                case 32:
                    kernel<32, true><<<batches, threads, 0, stream>>>(tmp_sums, elements, scale,
                                                                      output_variances, output_stddevs);
                    break;
                default:
                    NOA_THROW("DEV: block size should be 32, 64, 128 or 256, "
            ...
            switch (threads) {
                case 256:
                    kernel<256, false><<<batches, threads, 0, stream>>>(tmp_sums, elements, scale,
                                                                        output_variances, output_stddevs);
                    break;
                case 128:
                    kernel<128, false><<<batches, threads, 0, stream>>>(tmp_sums, elements, scale,
                                                                        output_variances, output_stddevs);
                    break;
                case 64:
                    kernel<64, false><<<batches, threads, 0, stream>>>(tmp_sums, elements, scale,
                                                                       output_variances, output_stddevs);
                    break;
                case 32:
                    kernel<32, false><<<batches, threads, 0, stream>>>(tmp_sums, elements, scale,
                                                                       output_variances, output_stddevs);
                    break;
                default:
                    NOA_THROW("DEV: block size should be 32, 64, 128 or 256, "
