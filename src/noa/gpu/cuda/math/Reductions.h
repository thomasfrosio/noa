...
namespace noa::cuda::math {
    ...
    template<typename T>
    NOA_IH void min(const T* inputs, size_t input_pitch, T* output_mins, size3_t shape, uint batches, Stream& stream) {
    ...
    ///                         This stream will be synchronized when the function returns.
    template<typename T>
    NOA_IH void max(const T* inputs, size_t input_pitch, T* output_maxs, size3_t shape, uint batches, Stream& stream) {
    ...
    /// \param input_pitch      Pitch of \p inputs, in elements.
    /// \param[out] outputs     On the \b device. Reduced arrays. One per batch.
    ...
    NOA_HOST void reduceAdd(const T* inputs, T* outputs, size_t elements,
                            uint nb_to_reduce, uint batches, Stream& stream);
