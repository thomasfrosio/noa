...
namespace noa::cuda::math {
    ...
    template void ewise<T,U,V,TRINARY,void>(const shared_t<T[]>&, dim4_t, U, U,                         \
                                            const shared_t<V[]>&, dim4_t, dim4_t, TRINARY, Stream&);    \
    template void ewise<T,U,U,V,TRINARY,void>(const shared_t<T[]>&, dim4_t,                             \
                                              const shared_t<U[]>&, dim4_t, U,                          \
                                              const shared_t<V[]>&, dim4_t, dim4_t, TRINARY, Stream&);  \
    template void ewise<T,U,U,V,TRINARY,void>(const shared_t<T[]>&, dim4_t, U,                          \
