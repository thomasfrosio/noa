...
namespace noa::cuda::math::details {
    enum : int {
        ARITH_ADD, ARITH_SUBTRACT, ARITH_MULTIPLY, ARITH_DIVIDE, ARITH_DIVIDE_SAFE
    };
...
namespace noa::cuda::math {
    ...
    /// \param[in] input        On the \b device. Input array to multiply.
    /// \param input_pitch      Pitch, in elements, of \p input.
    /// \param[in] value        Multiplier.
    /// \param[out] output      On the \b device. Multiplied array. Can be equal to \p input.
    ...
    NOA_IH void multiplyByValue(const T* input, size_t input_pitch, U value,
        ...
        details::arithByValue<details::ARITH_MULTIPLY>(input, input_pitch, value,
    ...
    /// \tparam T               (u)int, (u)long, (u)long long, float, double, cfloat_t, cdouble_t.
    /// \tparam U               If \p T is complex, \p U can be the corresponding value type. Otherwise, same as \p T.
    /// \param[in] inputs       On the \b device. Input array(s) to multiply. One array per batch.
    /// \param inputs_pitch     Pitch, in elements, of \p inputs.
    /// \param[in] values       On the \b device. Multipliers. One value per batch.
    /// \param[out] outputs     On the \b device. Multiplied array(s). One array per batch. Can be equal to \p inputs.
    ...
    /// For each batch, computes the element-wise multiplication between \p inputs and \p array.
    /// \tparam T               (u)int, (u)long, (u)long long, float, double, cfloat_t, cdouble_t.
    /// \tparam U               If \p T is complex, \p U can be the corresponding value type. Otherwise, same as \p T.
    /// \param[in] inputs       On the \b device. Input array(s) to multiply. One array per batch.
    /// \param inputs_pitch     Pitch, in elements, of \p inputs.
    ...
    NOA_IH void multiplyByArray(const T* inputs, size_t inputs_pitch,
                                ...
                                size3_t shape, size_t batches, Stream& stream) {
        NOA_PROFILE_FUNCTION();
        details::arithByArray<details::ARITH_MULTIPLY>(inputs, inputs_pitch,
                                                       array, array_pitch,
...
namespace noa::cuda::math {
    ...
    /// \note This function is asynchronous with respect to the host and may return before completion.
    template<typename T, typename U>
    NOA_IH void divideByValue(const T* input, size_t input_pitch, U value,
                              T* output, size_t output_pitch,
                              size3_t shape, Stream& stream) {
        NOA_PROFILE_FUNCTION();
        details::arithByValue<details::ARITH_DIVIDE>(input, input_pitch, value,
                                                     output, output_pitch, shape, stream);
    }
    ...
    /// For each batch, divides \p inputs by a single value.
    /// \see This function has the same features and restrictions than noa::cuda::math::multiplyByValue().
    /// \note This function is asynchronous with respect to the host and may return before completion.
    template<typename T, typename U>
    ...
    /// For each batch, divides \p inputs by a single value. Version for contiguous layouts.
    /// \note This function is asynchronous with respect to the host and may return before completion.
    template<typename T, typename U>
    NOA_IH void divideByValue(const T* inputs, const U* values,
                              T* outputs, size_t elements, size_t batches, Stream& stream) {
        NOA_PROFILE_FUNCTION();
        details::arithByValue<details::ARITH_DIVIDE>(inputs, values, outputs, elements, batches, stream);
    }

    /// For each batch, computes the element-wise division between \p inputs and \p array.
    /// \see This function has the same features and restrictions than noa::cuda::math::multiplyByValue().
    /// \note This function is asynchronous with respect to the host and may return before completion.
    template<typename T, typename U>
    NOA_IH void divideByArray(const T* inputs, size_t inputs_pitch,
                              ...
                              size3_t shape, size_t batches, Stream& stream) {
        NOA_PROFILE_FUNCTION();
        details::arithByArray<details::ARITH_DIVIDE>(inputs, inputs_pitch,
                                                     array, array_pitch,
                                                     outputs, outputs_pitch,
                                                     shape, batches, stream);
    }

    /// For each batch, computes the element-wise division between \p inputs and \p array. Version for contiguous layouts.
    /// \note This function is asynchronous with respect to the host and may return before completion.
    template<typename T, typename U>
    NOA_IH void divideByArray(const T* inputs, const U* array,
                              T* outputs, size_t elements, size_t batches, Stream& stream) {
        NOA_PROFILE_FUNCTION();
    ...
    /// \note This function is asynchronous with respect to the host and may return before completion.
    template<typename T, typename U>
    NOA_IH void divideSafeByArray(const T* inputs, const U* array,
                                  T* outputs, size_t elements, size_t batches, Stream& stream) {
        NOA_PROFILE_FUNCTION();
...
namespace noa::cuda::math {
    ...
    /// \note This function is asynchronous with respect to the host and may return before completion.
    template<typename T, typename U>
    NOA_IH void addValue(const T* input, size_t input_pitch, U value,
                         T* output, size_t output_pitch,
                         size3_t shape, Stream& stream) {
        NOA_PROFILE_FUNCTION();
        details::arithByValue<details::ARITH_ADD>(input, input_pitch, value,
                                                  output, output_pitch,
                                                  shape, stream);
    }
    ...
    /// For each batch, adds a single value to \p inputs.
    /// \see This function has the same features and restrictions than noa::cuda::math::multiplyByValue().
    /// \note This function is asynchronous with respect to the host and may return before completion.
    template<typename T, typename U>
    NOA_IH void addValue(const T* inputs, size_t inputs_pitch, const U* values,
                         T* outputs, size_t outputs_pitch,
                         size3_t shape, size_t batches, Stream& stream) {
        NOA_PROFILE_FUNCTION();
        details::arithByValue<details::ARITH_ADD>(inputs, inputs_pitch, values,
                                                  outputs, outputs_pitch,
                                                  shape, batches, stream);
    }

    /// For each batch, adds a single value to \p inputs. Version for contiguous layouts.
    /// \note This function is asynchronous with respect to the host and may return before completion.
    template<typename T, typename U>
    NOA_IH void addValue(const T* inputs, const U* values,
                         T* outputs, size_t elements, size_t batches, Stream& stream) {
        NOA_PROFILE_FUNCTION();
    ...
    NOA_IH void addArray(const T* inputs, size_t inputs_pitch,
                         ...
                         size3_t shape, size_t batches, Stream& stream) {
        NOA_PROFILE_FUNCTION();
        details::arithByArray<details::ARITH_ADD>(inputs, inputs_pitch,
                                                  array, array_pitch,
                                                  outputs, outputs_pitch,
                                                  shape, batches, stream);
    }
    ...
    /// \note This function is asynchronous with respect to the host and may return before completion.
    template<typename T, typename U>
    NOA_IH void addArray(const T* inputs, const U* array,
                         T* outputs, size_t elements, size_t batches, Stream& stream) {
        NOA_PROFILE_FUNCTION();
...
namespace noa::cuda::math {
    ...
    /// \note This function is asynchronous with respect to the host and may return before completion.
    template<typename T, typename U>
    NOA_IH void subtractValue(const T* input, size_t input_pitch, U value,
                              T* output, size_t output_pitch,
                              size3_t shape, Stream& stream) {
        NOA_PROFILE_FUNCTION();
        details::arithByValue<details::ARITH_SUBTRACT>(input, input_pitch, value,
                                                       output, output_pitch,
                                                       shape, stream);
    }
    ...
    /// For each batch, subtracts a single value to \p inputs.
    /// \see This function has the same features and restrictions than noa::cuda::math::multiplyByValue().
    /// \note This function is asynchronous with respect to the host and may return before completion.
    template<typename T, typename U>
    NOA_IH void subtractValue(const T* inputs, size_t inputs_pitch, const U* values,
                              T* outputs, size_t outputs_pitch,
                              size3_t shape, size_t batches, Stream& stream) {
        NOA_PROFILE_FUNCTION();
    ...
    /// For each batch, subtracts a single value to \p inputs. Version for contiguous layouts.
    /// \note This function is asynchronous with respect to the host and may return before completion.
    template<typename T, typename U>
    NOA_IH void subtractValue(const T* inputs, const U* values,
                              T* outputs, size_t elements,
                              size_t batches, Stream& stream) {
        NOA_PROFILE_FUNCTION();
        details::arithByValue<details::ARITH_SUBTRACT>(inputs, values, outputs, elements, batches, stream);
    }

    /// For each batch, subtracts \p array to \p inputs.
    /// \see This function has the same features and restrictions than noa::cuda::math::multiplyByArray().
    /// \note This function is asynchronous with respect to the host and may return before completion.
    template<typename T, typename U>
    ...
    /// For each batch, subtracts \p array to \p inputs. Version for contiguous layouts.
    /// \note This function is asynchronous with respect to the host and may return before completion.
    template<typename T, typename U>
