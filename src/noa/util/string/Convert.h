...
namespace Noa::String {
     ...
     */
    template<typename Int = int, typename S = std::string_view,
    ...
    NOA_HOST auto toInt(S&& str) {
        ...
        if constexpr (Noa::Traits::is_uint_v<int_t>) {
            ...
            static_assert(Noa::Traits::always_false_v<int_t>, "DEV: this should not be possible");
        }
    ...
    template<typename Float = float, typename S = std::string_view,
             typename = std::enable_if_t<Noa::Traits::is_float_v<Float> && Noa::Traits::is_string_v<S>>>
    NOA_HOST auto toFloat(S&& str) {
        errno = 0;
        char* end;

        using real_t = Noa::Traits::remove_ref_cv_t<Float>;
        real_t out{};
        if constexpr (Noa::Traits::is_same_v<real_t, float>) {
            out = std::strtof(str.data(), &end);
        } else if constexpr (Noa::Traits::is_same_v<real_t, double>) {
            out = std::strtod(str.data(), &end);
        } else {
            static_assert(Noa::Traits::always_false_v<real_t>);
        }

        if (end == str.data())
            NOA_THROW("Invalid data. Cannot convert \"{}\" to {}", str, String::typeName<real_t>());
        else if (errno == ERANGE)
            NOA_THROW("Out of range. Cannot convert \"{}\" to {}", str, String::typeName<real_t>());
        return out;
    }

    template<typename S, typename = std::enable_if_t<Noa::Traits::is_string_v<S>>>
    NOA_HOST bool toBool(S&& str) {
        if (str == "1" || str == "true")
            return true;
        else if (str == "0" || str == "false")
            return false;
