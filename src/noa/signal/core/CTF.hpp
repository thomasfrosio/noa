...
namespace noa::signal {
    ...
    private:
        NOA_HD void set_lambda_and_cs_() {
            ...
            m_k2 = static_cast<value_type>(PI * 0.5 * static_cast<f64>(m_cs_angstroms) * lambda * lambda * lambda);
        }
    ...
    public:
        ...
        [[nodiscard]] constexpr auto phase_at(const Vec<Coord, 2>& fftfreq) const -> value_type {
            ...
            const auto phi = nx::cartesian2phi<false>(scaled_fftfreq);
            const auto rho = nx::cartesian2rho(scaled_fftfreq);
        ...
        template<nt::real Coord>
        [[nodiscard]] constexpr auto isotropic_fftfreq(const Vec<Coord, 2>& fftfreq_2d) const -> value_type {
            ...
            const auto phi = nx::cartesian2phi<false>(scaled_fftfreq_2d);
            const auto rho = nx::cartesian2rho(scaled_fftfreq_2d);
...
namespace noa::traits {
    ...
    template<ctf T> struct proclaim_is_trivial_zero<T> : std::true_type {};
}
