...
namespace noa::transform {
    ...
    template<typename T>
    NOA_IHD constexpr Mat33 <T> scale(const Float3 <T>& s) noexcept {
        return Mat33<T>(s);
    }
    ...
    /// \return
    template<typename T>
    NOA_IHD constexpr Mat33 <T> rotate(const Float3 <T>& axis, T angle) noexcept {
    ...
    template<typename T>
    NOA_IHD constexpr Mat33 <T> rotateInPlane(T axis_angle, T angle) noexcept {
    ...
    NOA_IHD constexpr Mat33 <T> rotateX(T angle) noexcept {
        T c = math::cos(angle);
        T s = math::sin(angle);
        return Mat33<T>(1, 0, 0, 0, c, -s, 0, s, c);
    }
    ...
    NOA_IHD constexpr Mat33 <T> rotateY(T angle) noexcept {
        T c = math::cos(angle);
        T s = math::sin(angle);
        return Mat33<T>(c, 0, s, 0, 1, 0, -s, 0, c);
    }
    ...
    NOA_IHD constexpr Mat33 <T> rotateZ(T angle) noexcept {
        T c = math::cos(angle);
        T s = math::sin(angle);
        return Mat33<T>(c, -s, 0, s, c, 0, 0, 0, 1);
    }
    ...
    template<typename T>
    NOA_IHD constexpr Mat44 <T> translate(const Float3 <T>& v) noexcept {
        return Mat44<T>(1, 0, 0, v.x,
                        0, 1, 0, v.y,
                        0, 0, 1, v.z,
                        0, 0, 0, 1);
    }
    ...
    template<typename T>
    NOA_IHD constexpr Mat22 <T> scale(const Float2 <T>& s) noexcept {
        return Mat22<T>(s);
    }
    ...
    template<typename T>
    NOA_IHD constexpr Mat33 <T> translate(const Float2 <T>& v) noexcept {
        return Mat33<T>(1, 0, v.x,
                        0, 1, v.y,
                        0, 0, 1);
    }
}
