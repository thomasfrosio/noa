...
namespace noa {
    template<typename TTo, typename TFrom, typename = std::enable_if_t<traits::is_int3_v<TTo>>>
    [[nodiscard]] NOA_FHD constexpr TTo clamp_cast(const Int3<TFrom>& src) noexcept {
        using value_t = traits::value_type_t<TTo>;
        return {clamp_cast<value_t>(src[0]), clamp_cast<value_t>(src[1]), clamp_cast<value_t>(src[2])};
    }

    template<typename TTo, typename TFrom, typename = std::enable_if_t<traits::is_int3_v<TTo>>>
    [[nodiscard]] NOA_FHD constexpr bool isSafeCast(const Int3<TFrom>& src) noexcept {
        using value_t = traits::value_type_t<TTo>;
