...
#include "noa/common/traits/MatrixTypes.h"
...
namespace noa {
    ...
    // The indexing is "row-first" (as opposed to "column-first", like in OpenGL Math),
    // i.e. indexing is m[r][c], where r = row index and c = column index. All indexes starts from 0.
    // This matrix is quite limited compared to the squared ones, and it's mostly here
    ...
    public: // (Conversion) Constructors
        ...
        template<typename U>
        NOA_HD constexpr explicit Mat23(Vec2<U> v) noexcept
        ...
        NOA_HD constexpr explicit Mat23(Mat22<U> m, Vec2<V> v) noexcept
        ...
        NOA_HD constexpr Mat23(Vec3<V0> r0,
                               Vec3<V1> r1) noexcept
                : m_row{row_type(r0),
                        row_type(r1)} {}
    ...
    public: // Non-member functions
        // -- Unary operators --
        [[nodiscard]] friend NOA_HD constexpr Mat23 operator+(Mat23 m) noexcept {
            return m;
        }
        ...
        [[nodiscard]] friend NOA_HD constexpr Mat23 operator+(Mat23 m1, Mat23 m2) noexcept {
            return Mat23(m1[0] + m2[0], m1[1] + m2[1]);
        }

        [[nodiscard]] friend NOA_HD constexpr Mat23 operator+(value_type s, Mat23 m) noexcept {
            return Mat23(s + m[0], s + m[1]);
        }

        [[nodiscard]] friend NOA_HD constexpr Mat23 operator+(Mat23 m, value_type s) noexcept {
        ...
        [[nodiscard]] friend NOA_HD constexpr vec2_type operator*(Mat23 m, const vec3_type& column) noexcept {
            return vec2_type(math::dot(m[0], column), math::dot(m[1], column));
        }

        [[nodiscard]] friend NOA_HD constexpr vec3_type operator*(const vec2_type& row, Mat23 m) noexcept {
        ...
        [[nodiscard]] friend NOA_HD constexpr bool operator==(Mat23 m1, Mat23 m2) noexcept {
...
namespace noa::math {
    ...
    [[nodiscard]] NOA_IHD constexpr bool are_almost_equal(Mat23<T> m1, Mat23<T> m2, T epsilon = 1e-6f) noexcept {
