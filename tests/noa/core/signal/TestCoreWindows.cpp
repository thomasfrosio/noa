...
TEST_CASE("core::signal:: windows", "[noa][core]") {
...

    {
        ...
        const std::array<f64, 20> window_blackman_numpy{
        ...
        REQUIRE(test::Matcher(test::MATCH_ABS, window.data(), window_blackman_numpy.data(), 20, 1e-8));

        noa::signal::window_blackman(window.data(), 20, /*normalize=*/ false, /*half_window=*/ true);
        REQUIRE(test::Matcher(test::MATCH_ABS, window.data(), window_blackman_numpy.data() + 10, 10, 1e-8));
    }

    {
        std::array<f64, 21> window{};
        noa::signal::window_blackman(window.data(), 21, /*normalize=*/ false, /*half_window=*/ false);
            const std::array<f64, 21> window_blackman_numpy{
        ...
        REQUIRE(test::Matcher(test::MATCH_ABS, window.data(), window_blackman_numpy.data(), 21, 1e-8));

        noa::signal::window_blackman(window.data(), 21, /*normalize=*/ false, /*half_window=*/ true);
        REQUIRE(test::Matcher(test::MATCH_ABS, window.data(), window_blackman_numpy.data() + 10, 11, 1e-8));
    }

    {
        std::array<f64, 20> window{};
        noa::signal::window_sinc(window.data(), 20, 1., /*normalize=*/ false, /*half_window=*/ false);
        const std::array<f64, 20> window_sinc_numpy{
        ...
        REQUIRE(test::Matcher(test::MATCH_ABS, window.data(), window_sinc_numpy.data(), 20, 1e-8));

        noa::signal::window_sinc(window.data(), 20, 1., /*normalize=*/ false, /*half_window=*/ true);
        REQUIRE(test::Matcher(test::MATCH_ABS, window.data(), window_sinc_numpy.data() + 10, 10, 1e-8));
    }

    {
        std::array<f64, 21> window{};
        noa::signal::window_sinc(window.data(), 21, 0.33, /*normalize=*/ false, /*half_window=*/ false);
        const std::array<f64, 21> window_sinc_numpy{
        ...
        REQUIRE(test::Matcher(test::MATCH_ABS, window.data(), window_sinc_numpy.data(), 21, 1e-8));

        noa::signal::window_sinc(window.data(), 21, 0.33, /*normalize=*/ false, /*half_window=*/ true);
        REQUIRE(test::Matcher(test::MATCH_ABS, window.data(), window_sinc_numpy.data() + 10, 11, 1e-8));
    }
}
