...
TEST_CASE("core::geometry::Quaternion") {
    ...
    AND_THEN("new") {
        ...
        REQUIRE(noa::all(q0.coefficients() == Vec{1., 2., 3., 4.}));
    ...
    AND_THEN("rotate") {
        ...
        auto a0 = m * v;
        REQUIRE(noa::all(noa::allclose(a0, Vec{0., 1., 0.})));

        auto q1 = Quaternion<f64>::from_matrix(m);
        auto a1 = q1 * Quaternion<f64>::from_coefficients(v.push_back(0)) * q1.conj();
        auto a2 = q1.rotate(v);

        REQUIRE(noa::all(noa::allclose(a1.coefficients(), Vec{0., 1., 0., 0.})));
        REQUIRE(noa::all(noa::allclose(a2, Vec{0., 1., 0.})));
    }
    ...
    AND_THEN("rotations") {
        ...
        for (int i = 0; i < 1000; ++i) {
            ...
            REQUIRE(noa::all(noa::allclose(v0, v1)));
            REQUIRE(noa::all(noa::allclose(v0, v2)));
            REQUIRE(noa::all(noa::allclose(v0, v3)));
        }
    }
}
