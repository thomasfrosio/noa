...
TEMPLATE_TEST_CASE("runtime::arange(), cpu", "", i32, i64, u32, u64, f32, f64, c32, c64) {
...
TEMPLATE_TEST_CASE("runtime:: factories, cpu vs gpu", "", i32, i64, u32, u64, f64, c64) {
...
TEST_CASE("runtime:: Factory functions") {
    using namespace noa::types;
                ...
                for (i64 k{}; k < span.shape()[2]; ++k)
                    for (i64 l{}; l < span.shape()[3]; ++l)
                        span(i, j, k, l) = static_cast<f64>(
                                noa::offset_at(expected.strides(), Vec{i, j, k, l} % tile.as<i64>()));
