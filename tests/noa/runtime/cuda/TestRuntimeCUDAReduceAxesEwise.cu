...
TEST_CASE("runtime::cuda::reduce_axes_ewise") {
    ...
    SECTION("sum over one axis") {
        std::array input_shapes{Shape4{10, 34, 65, 130}, test::random_shape_batched(4)};
        for (auto& input_shape: input_shapes) {
            ...
            auto input = noa::make_tuple(Accessor<f32, 4>(input_buffer.get(), input_shape.strides()));
            ...
            for (auto i: noa::irange(4)) {
                ...
                auto output = noa::make_tuple(Accessor<f32, 4>(output_buffer.get(), output_shape.strides()));
    ...
    SECTION("sum per batch") {
        const std::array input_shapes{Shape4{6, 8, 13, 99}, Shape4{4, 35, 55, 130}};
        for (const auto& input_shape: input_shapes) {
            ...
            auto output = noa::make_tuple(Accessor<f64, 4>(output_buffer.get(), output_shape.strides()));
            auto init = noa::make_tuple(noa::AccessorValue<f64>(0.));
            auto reduce_op = []__device__(f64 to_reduce, f64& reduced) { reduced += to_reduce; };

            auto input_contiguous = noa::make_tuple(Accessor<f64, 4>(input_buffer.get(), input_shape.strides()));
            ...
            auto input_strided = noa::make_tuple(Accessor<f64, 4>(input_buffer.get(), strided));
