...
TEST_CASE("runtime::cpu::reduce_ewise") {
    AND_THEN("simple sum") {
        ...
        REQUIRE_THAT(output[Tag<0>{}].ref(), Catch::Matchers::WithinAbs(static_cast<f64>(elements), 1e-8));
    }
    ...
    AND_THEN("sum-max") {
        ...
        REQUIRE_THAT(output[Tag<0>{}].ref(), Catch::Matchers::WithinAbs(static_cast<f64>(elements + 12), 1e-8));
        REQUIRE_THAT(output[Tag<1>{}].ref(), Catch::Matchers::WithinAbs(12., 1e-8));

        reduce_ewise<ReduceEwiseConfig<false, false, true>>(shape, std::move(reduce_op), input, reduced, output);
        REQUIRE_THAT(output[Tag<0>{}].ref(), Catch::Matchers::WithinAbs(static_cast<f64>(elements + 11), 1e-8));
        REQUIRE_THAT(output[Tag<1>{}].ref(), Catch::Matchers::WithinAbs(13., 1e-8));
    }
}
