...
TEST_CASE("xform::Quaternion") {
    ...
    AND_THEN("rotate") {
        ...
        REQUIRE(noa::allclose(a1.to_coefficients(), Vec{0., 1., 0., 0.}));
    ...
    AND_THEN("rotations") {
        ...
        for (int i = 0; i < 1000; ++i) {
            ...
            const auto rotation_matrix = euler2matrix(euler_angles);
            ...
            const auto v2 =
                (quaternion * Quaternion<f64>::from_coefficients(v.push_back(0)) * quaternion.conj())
                .to_coefficients().pop_back();
