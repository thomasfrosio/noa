...
TEST_CASE("xform::transform_spectrum_2d, vs scipy", "[asset]") {
    ...
    for (size_t nb{}; nb < param["tests"].size(); ++nb) {
        ...
        const auto matrix = (nx::rotate(rotate) * nx::scale(1 / scale)).inverse().as<f32>();
...
TEST_CASE("xform::transform_spectrum_3d, vs scipy", "[asset]") {
    ...
    for (size_t nb{}; nb < param["tests"].size(); ++nb) {
        ...
        const auto matrix = (nx::euler2matrix(rotate) * nx::scale(1 / scale)).inverse().as<f32>();
...
TEMPLATE_TEST_CASE("xform::transform_spectrum_2d(), remap", "", f32, f64) {
    ...
    for (auto& transform: transforms.span_1d()) {
        ...
        transform = (nx::rotate(angle) * nx::scale(1 / scale)).inverse().as<f32>();
    }
...
TEMPLATE_TEST_CASE("xform::transform_spectrum_3d(), remap", "", f32, f64) {
    ...
    for (auto& transform: transforms.span_1d()) {
        ...
        transform = (nx::euler2matrix(angles) * nx::scale(1 / scale)).inverse().as<f32>();
    }
    for (auto& shift: shifts.span_1d()) {
        shift = Vec{
            randomizer.get() * 10,
...
//         rotation = nx::rotate(noa::deg2rad(i));
//         i += 10;
//     }
//
//     auto output_rfft = noa::empty<f32>(shape.set<0>(batch).rfft());
//     nx::transform_spectrum_2d<"H2HC">(
...
//     nx::draw_shape({}, input, nx::Rectangle{
//         .center = center,
//         .radius = Vec{64., 64.},
//         .smoothness = 2.,
//     });
