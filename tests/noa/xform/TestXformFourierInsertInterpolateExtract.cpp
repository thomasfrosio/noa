...
TEST_CASE("xform::insert_and_extract_central_slices_3d", "[asset]") {
    ...
    for (usize nb{}; nb < tests["tests"].size(); ++nb) {
        INFO("test number = " << nb);

        const YAML::Node& parameters = tests["tests"][nb];

        const auto input_slice_shape = parameters["input_slice_shape"].as<Shape3>();
        const auto output_slice_shape = parameters["output_slice_shape"].as<Shape3>();
        ...
        for (usize i{}; auto& input_inv_rotation_matrix: input_inv_rotation_matrices.span())
            input_inv_rotation_matrix = nx::rotate_y(noa::deg2rad(-input_rotate[i++]));
        for (usize i{}; auto& output_fwd_rotation_matrix: output_fwd_rotation_matrices.span())
        ...
        for (auto& device: devices) {
            ...
            const Array asset_output_slice_fft = noa::read_image<f32, 3>(output_slice_filename).data;
