...
TEST_CASE("io::cast") {
    ...
    for (auto extension: std::array{".mrc", ".tiff"}) {
...

        {
            ...
            noa::write_image(a0, file_0, {.rank = 2, .spacing = s0, .dtype = dtype});
            noa::write_image(a0.as<const std::byte>(), "f32", file_1, {.rank = 2, .spacing = s0, .dtype = dtype});

            auto a1 = nio::read_image<f32, 3>(file_0, {.rank = 2}).data;
            auto a2 = nio::read_image<std::byte, 1>(file_1, {.rank = 2, .dtype = "f32"}).data;
