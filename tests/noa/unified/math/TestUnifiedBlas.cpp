...
namespace {
    ...
    void naive_matmul_(const Array<T>& lhs, const Array<T>& rhs, const Array<T>& out) {
        ...
        for (i64 batch = 0; batch < bmnk[0]; ++batch) {
            for (i64 m = 0; m < bmnk[1]; ++m) {
                for (i64 n = 0; n < bmnk[2]; ++n) {
                    for (i64 k = 0; k < bmnk[3]; ++k) {
...
TEMPLATE_TEST_CASE("unified::math::matmul()", "[noa][unified]", f32, f64, c32, c64) {
    ...
    for (auto& device: devices) {
        ...
        // Compute expected:
        const auto expected = noa::memory::empty<TestType>(out_shape, options);
