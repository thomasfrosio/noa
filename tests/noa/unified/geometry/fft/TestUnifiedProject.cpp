...
//             inv_rotation_matrix = ng::euler2matrix(
//                 noa::deg2rad(Vec{0.f, -rotate[i++], 0.f}), {.axes = "zyx"});
//
//         for (auto& device: devices) {
//             const auto stream = StreamGuard(device);
...
//             }
//
//             noa::write(volume_fft, path / "test_fourier_insert.mrc");
//             const Array asset_volume_fft = noa::read_data<f32>(volume_filename);
...
TEST_CASE("unified::geometry::fourier_insert_interpolate_3d, 2", "[noa][unified][asset]") {
    const Path path = test::NOA_DATA_PATH / "geometry" / "fft";

    constexpr auto slice_shape = Shape4<i64>{1, 1, 256, 256};
    constexpr auto volume_shape = Shape4<i64>{1, 256, 256, 256};
    const auto inv_rotation_matrices = ng::euler2matrix(noa::deg2rad(Vec{45., 0., 0.}), {.axes="zyx"});

    Stream::current("cpu").set_thread_limit(1);

    const Array slice_rfft = noa::ones<f32>(slice_shape.rfft());
    const Array volume_rfft = noa::zeros<f32>(volume_shape.rfft());
    ng::fourier_insert_interpolate_3d<Remap::HC2HC>(
        slice_rfft, slice_rfft, slice_shape,
        volume_rfft, {}, volume_shape,
        {}, inv_rotation_matrices);

    noa::write(volume_rfft, path / "test_fourier_insert.mrc");
}
...
//     const auto windowed_sinc = ng::WindowedSinc{0.02, 0.06};
//
