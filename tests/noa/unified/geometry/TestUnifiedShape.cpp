...
TEST_CASE("unified::geometry::sphere(), 2d", "[asset][noa][unified]") {
    ...
    for (size_t nb = 0; nb < tests.size(); ++nb) {
        ...
        if constexpr (COMPUTE_ASSETS) {
            ...
            noa::geometry::draw_shape({}, asset, sphere);
        ...
        for (const auto& device: devices) {
            ...
            noa::geometry::draw_shape({}, result, sphere);
            ...
            noa::geometry::draw_shape(data, data, sphere);
            ...
            noa::geometry::draw_shape(data, data, inverted_sphere);
...
TEST_CASE("unified::geometry::rectangle(), 2d", "[assets][noa][unified]") {
    ...
    for (size_t nb = 0; nb < tests.size(); ++nb) {
        ...
        if constexpr (COMPUTE_ASSETS) {
            const auto asset = noa::empty<f32>(shape);
            noa::geometry::draw_shape({}, asset, rectangle, inv_matrix);
            noa::io::write(asset, filename_expected);
            continue;
        }
        ...
        for (auto device: devices) {
            ...
            // Save shape into the output.
            const auto result = noa::empty<f32>(shape, option);
            noa::geometry::draw_shape({}, result, rectangle, inv_matrix);
            REQUIRE(test::allclose_abs(result, asset, 5e-5));

            // Apply the shape onto the input, in-place.
            const auto data = noa::random(noa::Uniform{-10.f, 10.f}, shape, option);
            noa::ewise(noa::wrap(result, data), result, noa::Multiply{});
            noa::geometry::draw_shape(data, data, rectangle, inv_matrix);
            REQUIRE(test::allclose_abs(result, data, 5e-5));

            // Apply the shape onto the input, in-place, invert.
            noa::randomize(noa::Uniform{-10.f, 10.f}, data);
            noa::ewise(noa::wrap(data, asset), asset, ApplyInvertedMask{static_cast<f32>(cvalue)});
            noa::geometry::draw_shape(data, data, inverted_rectangle, inv_matrix);
            REQUIRE(test::allclose_abs(asset, data, 5e-5));
        }
    }
}
...
TEST_CASE("unified::geometry::ellipse(), 2d", "[assets][noa][unified]") {
    ...
    for (size_t nb = 0; nb < tests.size(); ++nb) {
        ...
        if constexpr (COMPUTE_ASSETS) {
            const auto asset = noa::empty<f32>(shape);
            noa::geometry::draw_shape({}, asset, ellipse, inv_matrix);
            noa::io::write(asset, filename_expected);
            continue;
        }
        ...
        for (auto device: devices) {
            ...
            // Save shape into the output.
            const auto result = noa::empty<f32>(shape, option);
            noa::geometry::draw_shape({}, result, ellipse, inv_matrix);
            REQUIRE(test::allclose_abs(result, asset, 5e-5));

            // Apply the shape onto the input, in-place.
            const auto data = noa::random(noa::Uniform{-10.f, 10.f}, shape, option);
            noa::ewise(noa::wrap(result, data), result, noa::Multiply{});
            noa::geometry::draw_shape(data, data, ellipse, inv_matrix);
            REQUIRE(test::allclose_abs(result, data, 5e-4));

            // Apply the shape onto the input, in-place, invert.
            noa::randomize(noa::Uniform{-10.f, 10.f}, data);
            noa::ewise(noa::wrap(data, asset), asset, ApplyInvertedMask{static_cast<f32>(cvalue)});
            noa::geometry::draw_shape(data, data, inverted_ellipse, inv_matrix);
...
TEST_CASE("unified::geometry::cylinder", "[assets][noa][unified]") {
    ...
    for (size_t nb = 0; nb < tests.size(); ++nb) {
        ...
        if constexpr (COMPUTE_ASSETS) {
            const auto asset = noa::empty<f32>(shape);
            noa::geometry::draw_shape({}, asset, cylinder, inv_matrix);
            noa::io::write(asset, filename_expected);
            continue;
        }
        ...
        for (auto device: devices) {
            ...
            // Save shape into the output.
            const auto result = noa::empty<f32>(shape, option);
            noa::geometry::draw_shape({}, result, cylinder, inv_matrix);
            REQUIRE(test::allclose_abs(result, asset, 5e-5));

            // Apply the shape onto the input, in-place.
            const auto data = noa::random(noa::Uniform{-10.f, 10.f}, shape, option);
            noa::ewise(noa::wrap(result, data), result, noa::Multiply{});
            noa::geometry::draw_shape(data, data, cylinder, inv_matrix);
            REQUIRE(test::allclose_abs(result, data, 5e-5));

            // Apply the shape onto the input, in-place, invert.
            noa::randomize(noa::Uniform{-10.f, 10.f}, data);
            noa::ewise(noa::wrap(data, asset), asset, ApplyInvertedMask{static_cast<f32>(cvalue)});
            noa::geometry::draw_shape(data, data, inverted_cylinder, inv_matrix);
            REQUIRE(test::allclose_abs(asset, data, 5e-5));
        }
    }
}
...
TEMPLATE_TEST_CASE("unified::geometry::rectangle, 2d matches 3d", "[noa][unified]", f32, f64, c32, c64) {
    ...
    for (auto device: devices) {
        ...
        noa::geometry::draw_shape(input, output_2d, rectangle_2d, inv_transform_2d);
        noa::geometry::draw_shape(input, output_3d, rectangle_3d, inv_transform_3d);
        REQUIRE(test::allclose_abs(output_2d, output_3d, 1e-4));
    }
}
...
TEMPLATE_TEST_CASE("unified::geometry::ellipse, 2d matches 3d", "[noa][unified]", f32, f64, c32, c64) {
    ...
    for (auto device: devices) {
        ...
        noa::geometry::draw_shape(input, output_2d, ellipse_2d, inv_transform_2d);
        noa::geometry::draw_shape(input, output_3d, ellipse_3d, inv_transform_3d);
        REQUIRE(test::allclose_abs(output_2d, output_3d, 1e-4));
    }
}
...
TEMPLATE_TEST_CASE("unified::geometry::ellipse, 2d affine", "[noa][unified]", f32, f64, c32, c64) {
    ...
    for (auto device: devices) {
        ...
        noa::geometry::draw_shape(input, output_linear, ellipse, inv_matrix_linear);
        noa::geometry::draw_shape(input, output_affine, ellipse, inv_matrix_affine);
    ...
    for (auto device: devices) {
        ...
        AND_THEN("ellipse") {
            ...
            for (i64 i{}; i < shape[0]; ++i)
                noa::geometry::draw_shape({}, serial.subregion(i), ellipse, matrices(0, 0, 0, i));
            noa::geometry::draw_shape({}, batched, ellipse, matrices_device);
        ...
        AND_THEN("sphere") {
            ...
            for (i64 i{}; i < shape[0]; ++i)
                noa::geometry::draw_shape({}, serial.subregion(i), sphere, matrices(0, 0, 0, i));
            noa::geometry::draw_shape({}, batched, sphere, matrices_device);
            REQUIRE(test::allclose_abs_safe(batched, serial, 1e-6));
        }
        ...
        AND_THEN("rectangle") {
            ...
            for (i64 i{}; i < shape[0]; ++i)
                noa::geometry::draw_shape({}, serial.subregion(i), rectangle, matrices(0, 0, 0, i));
            noa::geometry::draw_shape({}, batched, rectangle, matrices_device);
            REQUIRE(test::allclose_abs_safe(batched, serial, 1e-6));
        }
    }
}
...
TEST_CASE("unified::geometry::shapes, 3d batched", "[noa][unified]") {
    ...
    for (auto device: devices) {
        ...
        AND_THEN("cylinder") {
            ...
            for (i64 i = 0; i < shape[0]; ++i)
                noa::geometry::draw_shape({}, serial.subregion(i), cylinder, matrices(0, 0, 0, i));
            noa::geometry::draw_shape({}, batched, cylinder, matrices_device);
            REQUIRE(test::allclose_abs_safe(batched, serial, 1e-6));
        }
    }
}
