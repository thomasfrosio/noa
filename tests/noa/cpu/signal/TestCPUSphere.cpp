...
TEST_CASE("cpu::signal::sphere(), 2D", "[assets][noa][cpu]") {
    ...
    for (size_t nb = 0; nb < tests.size(); ++nb) {
        ...
        cpu::signal::sphere<float>(nullptr, {}, mask_result.share(), strides, shape,
        ...
        AND_THEN("invert = true") {
            ...
            cpu::signal::sphere<float>(input_result.share(), strides, input_result.share(), strides, shape,
...
TEST_CASE("cpu::signal::sphere(), 3D", "[assets][noa][cpu]") {
    test::Randomizer<float> randomizer(-5, 5);
...
TEMPLATE_TEST_CASE("cpu::signal::sphere(), 2D matches 3D", "[assets][noa][cpu]",
    ...
    cpu::Stream stream;
    cpu::signal::sphere(input.share(), strides, output_2d.share(), strides, shape,
                        center_2d, radius, edge_size, math::inverse(fwd_transform_2d), invert, stream);
    cpu::signal::sphere(input.share(), strides, output_3d.share(), strides, shape,
