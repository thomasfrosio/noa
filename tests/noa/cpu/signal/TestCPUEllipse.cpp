...
TEST_CASE("cpu::signal::ellipse()", "[assets][noa][cpu]") {
    ...
    for (size_t nb = 0; nb < tests.size(); ++nb) {
        ...
        if constexpr (GENERATE_ASSETS) {
            ...
            if (invert)
                cpu::signal::ellipse<true, float>(nullptr, {}, mask.share(), strides, shape, center, radius, taper, stream);
            else
                cpu::signal::ellipse<false, float>(nullptr, {}, mask.share(), strides, shape, center, radius, taper, stream);
            stream.synchronize();
        ...
        AND_THEN("invert = true") {
            ...
            cpu::signal::ellipse<true, float>(
                    nullptr, {}, mask_result.share(), strides, shape, center, radius, taper, stream);
            REQUIRE(test::Matcher(test::MATCH_ABS, mask_expected.get(), mask_result.get(), elements, 1e-6));

            // Test on-the-fly, in-place.
            cpu::signal::ellipse<true, float>(
                    input_result.share(), strides, input_result.share(), strides, shape, center, radius, taper, stream);
        ...
        AND_THEN("invert = false") {
            ...
            cpu::signal::ellipse<false, float>(
                    nullptr, {}, mask_result.share(), strides, shape, center, radius, taper, stream);
            REQUIRE(test::Matcher(test::MATCH_ABS, mask_expected.get(), mask_result.get(), elements, 1e-6));
