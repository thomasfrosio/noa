...
TEST_CASE("cpu::signal::cylinder()", "[assets][noa][cpu][filter]") {
    ...
    for (size_t nb = 0; nb < tests.size(); ++nb) {
        ...
        AND_THEN("invert = false") {
            ...
            cpu::signal::cylinder<false, float>(nullptr, {}, mask_result.share(), stride, shape,
                                                center, radius, length, taper, stream);
            REQUIRE(test::Matcher(test::MATCH_ABS, mask_expected.get(), mask_result.get(), elements, 1e-4));

            // Test on-the-fly, in-place.
            cpu::signal::cylinder<false, float>(input_result.share(), stride, input_result.share(), stride, shape,
                                                center, radius, length, taper, stream);
            for (size_t idx = 0; idx < elements; ++idx)
        ...
        AND_THEN("invert = true") {
            ...
            cpu::signal::cylinder<true, float>(
                    nullptr, {}, mask_result.share(), stride, shape, center, radius, length, taper, stream);
            REQUIRE(test::Matcher(test::MATCH_ABS, mask_expected.get(), mask_result.get(), elements, 1e-4));

            // Test on-the-fly, in-place.
            cpu::signal::cylinder<true, float>(input_result.share(), stride, input_result.share(), stride, shape,
                                               center, radius, length, taper, stream);
            for (size_t idx = 0; idx < elements; ++idx)
                input_expected[idx] *= mask_expected[idx];
            REQUIRE(test::Matcher(test::MATCH_ABS, input_result.get(), input_expected.get(), elements, 1e-4));
