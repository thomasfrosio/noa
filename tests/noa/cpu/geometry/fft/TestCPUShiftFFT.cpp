...
TEST_CASE("cpu::transform::fft::shift2D()", "[assets][noa][cpu][transform]") {
    ...
    const auto pitch = shapeFFT(shape);
    ...
    cpu::transform::fft::shift2D<fft::H2H>(
            input.get(), pitch, input.get(), pitch, shape, shift, 1, stream); // in-place if no remap is OK
...
TEMPLATE_TEST_CASE("cpu::transform::fft::shift2D(), h2hc", "[noa][cpu][transform]", cfloat_t, cdouble_t) {
    ...
    cpu::Stream stream;
    cpu::transform::fft::shift2D<fft::H2H>(input.get(), pitch_2d, output.get(), pitch_2d, shape_2d, shift, 1, stream);
    ...
    cpu::transform::fft::shift2D<fft::H2HC>(
            input.get(), pitch_2d, output_centered.get(), pitch_2d, shape_2d, shift, 1, stream);
...
TEMPLATE_TEST_CASE("cpu::transform::fft::shift2D(), hc2h", "[noa][cpu][transform]", cfloat_t, cdouble_t) {
    ...
    cpu::Stream stream;
    cpu::transform::fft::shift2D<fft::H2H>(
            input.get(), pitch_2d, output.get(), pitch_2d, shape_2d, shift, 1, stream);
    ...
    cpu::transform::fft::shift2D<fft::HC2H>(
            input.get(), pitch_2d, output_2.get(), pitch_2d, shape_2d, shift, 1, stream);
...
TEMPLATE_TEST_CASE("cpu::transform::fft::shift3D(), h2hc", "[noa][cpu][transform]", cfloat_t, cdouble_t) {
    ...
    cpu::Stream stream;
    cpu::transform::fft::shift3D<fft::H2H>(input.get(), pitch, output.get(), pitch, shape, shift, 1, stream);
    cpu::fft::remap(fft::H2HC, output.get(), pitch, output.get(), pitch, shape, 1, stream);

    cpu::memory::PtrHost<TestType> output_centered(input.elements());
    cpu::transform::fft::shift3D<fft::H2HC>(input.get(), pitch, output_centered.get(), pitch, shape, shift, 1, stream);
...
TEMPLATE_TEST_CASE("cpu::transform::fft::shift3D(), hc2h", "[noa][cpu][transform]", cfloat_t, cdouble_t) {
    ...
    cpu::memory::PtrHost<TestType> output_2(input.elements());
    cpu::fft::remap(fft::H2HC, input.get(), pitch, input.get(), pitch, shape, 1, stream);
    cpu::transform::fft::shift3D<fft::HC2H>(input.get(), pitch, output_2.get(), pitch, shape, shift, 1, stream);
