...
TEST_CASE("memory::transpose()", "[noa][cpu][memory]") {
    ...
    if (in_place) {
        memory::transpose(data.get(), shape, data.get(), permutation, 1);
        float diff = test::getDifference(expected.get(), data.get(), elements);
        REQUIRE(diff == 0);
    } else {
        memory::transpose(data.get(), shape, result.get(), permutation, 1);
