...
TEMPLATE_TEST_CASE("cpu::memory::arange()", "[noa][cpu][memory]",
    ...
    cpu::Stream stream;
    {
        ...
        cpu::memory::arange(results.get(), elements, elements, batches, stream);
...

    {
        ...
        cpu::memory::arange(results.get(), elements, elements, batches, TestType(3), TestType(5), stream);
        TestType v = 3;
...
TEST_CASE("cpu::memory::linspace()", "[noa][cpu][memory]") {
    cpu::Stream stream;
    {
        ...
        cpu::memory::PtrHost<double> results(elements * batches);

        cpu::memory::linspace(results.get(), elements, elements, batches, 0., 5., false, stream);
        std::array<double, 5> expected = {0, 1, 2, 3, 4};

        REQUIRE(test::Matcher(test::MATCH_ABS, results.get(), expected.data(), elements, 1e-7));
        REQUIRE(test::Matcher(test::MATCH_ABS, results.get() + elements, expected.data(), elements, 1e-7));
    }

    {
        ...
        double step = cpu::memory::linspace(results.get(), elements, elements, batches, 0., 5.);
...

    {
        ...
        double step = cpu::memory::linspace(results.get(), elements, elements, batches, 0., 1.);
...

    {
        ...
        double step = cpu::memory::linspace(results.get(), elements, elements, batches, 0., 1., false);
...

    {
        ...
        double step = cpu::memory::linspace(results.get(), elements, elements, batches, 3., 40.);
