...
TEST_CASE("signal::ctf_isotropic, assets", "[asset]") {
    ...
    for (size_t nb = 0; nb < tests.size(); ++nb) {
        ...
        if constexpr (COMPUTE_ASSETS) {
            const auto input = noa::empty<f32, 4>(shape.rfft());
            ...
            for (auto device: devices) {
                ...
                const auto result_fft = noa::empty<f32, 4>(shape, options);
                const auto result_rfft = noa::empty<f32, 4>(shape.rfft(), options);
                const auto expected_rfft = noa::read_image<f32, 4>(filename_asset, {}, options).data;
...
TEST_CASE("signal::ctf_isotropic, default range") {
    ...
    for (auto device: devices) {
        ...
        const auto expected_0 = noa::empty_like(input);
        const auto expected_1 = noa::empty_like(input);
...
TEST_CASE("signal::ctf_isotropic, batches, cpu vs gpu") {
    if (not Device::is_any_gpu())
        return;

    auto shapes = noa::make_tuple(
        Pair{test::random_shape_batched<isize, 2>(1), Tag<1>{}},
        Pair{test::random_shape_batched<isize, 3>(1), Tag<1>{}},
        Pair{test::random_shape_batched<isize, 3>(2), Tag<2>{}},
        Pair{test::random_shape_batched<isize, 4>(1), Tag<1>{}},
        Pair{test::random_shape_batched<isize, 4>(2), Tag<2>{}},
        Pair{test::random_shape_batched<isize, 4>(3), Tag<3>{}},
        Pair{test::random_shape_batched<isize, 6>(1), Tag<1>{}},
        Pair{test::random_shape_batched<isize, 6>(2), Tag<2>{}},
        Pair{test::random_shape_batched<isize, 6>(3), Tag<3>{}}
    );
    ...
    shapes.for_each([]<usize N, usize R>(const Pair<Shape<isize, N>, Tag<R>> pair) {
        ...
        using CTFIsotropic64 = noa::signal::CTFIsotropic<f64>;
        const auto cpu_ctfs = noa::empty<CTFIsotropic64, B>(shape.template pop_back<R>());
        for (f64 offset{}; auto& ctf: cpu_ctfs.span_1d()) {
        ...
        noa::signal::ctf_isotropic<"h", R>({}, gpu_output, shape, gpu_ctfs); // compile-time rank
        REQUIRE(test::allclose_abs(cpu_output, gpu_output, 1e-4));
    });
}
...
TEST_CASE("signal::ctf_isotropic, range") {
    ...
    for (auto device: devices) {
        ...
        const auto output_truncated = output.subregion(trimmed_slice);
        ...
        noa::signal::ctf_isotropic<"H2H">(
            output_range, Shape1{trimmed_size}, ctf, {
                .fftfreq_range = {
...
TEST_CASE("signal::ctf_anisotropic, assets", "[asset]") {
    ...
    for (size_t nb = 0; nb < tests.size(); ++nb) {
        ...
        const auto& test = tests[nb];
        const auto filename_asset = path_base / test["output"].as<Path>();
        const auto shape = test["shape"].as<Shape4>().pop_front<2>();
        ...
        if constexpr (COMPUTE_ASSETS) {
            ...
            noa::signal::ctf_anisotropic_2d<"FC2FC">({}, input, shape, ctf);
            noa::write_image(input, filename_asset);
        } else {
            for (auto device: devices) {
                INFO(device);
                ...
                const auto expected_fft = noa::read_image<f32, 2>(filename_asset, {}, options).data;

                // Check against asset.
                noa::signal::ctf_anisotropic_2d<"FC2FC">(result_fft, shape, ctf);
                REQUIRE(test::allclose_abs(result_fft, expected_fft, 1e-4));

                // Remap
                noa::signal::ctf_anisotropic_2d<"F2F">(result_fft, shape, ctf);
                auto expected_f = noa::fft::remap_2d("FC2F", expected_fft, shape);
                REQUIRE(test::allclose_abs(result_fft, expected_f.drop(), 1e-4));
                ...
                if (defocus_astigmatic.angle == 0) {
                    noa::signal::ctf_anisotropic_2d<"H2H">(result_rfft, shape, ctf);
                    auto expected_r = noa::fft::remap_2d("FC2H", expected_fft, shape);
                    REQUIRE(test::allclose_abs(result_rfft, expected_r.drop(), 1e-4));

                    noa::signal::ctf_anisotropic_2d<"HC2HC">(result_rfft, shape, ctf);
                    auto expected_hc = noa::fft::remap_2d("FC2HC", expected_fft, shape);
                    REQUIRE(test::allclose_abs(result_rfft, expected_hc.drop(), 1e-4));
                }
                ...
                auto input_rfft = noa::random(noa::Normal<f32>{-5, 5}, shape.rfft(), options);
                noa::signal::ctf_anisotropic_2d<"HC2HC">({}, result_rfft, shape, ctf);
                auto input_rfft_centered = noa::fft::remap_2d("H2HC", input_rfft, shape);
                noa::ewise(noa::wrap(input_rfft_centered, result_rfft), input_rfft_centered, noa::Multiply{});
                noa::signal::ctf_anisotropic_2d<"H2HC">(input_rfft.copy(), input_rfft, shape, ctf);
                REQUIRE(test::allclose_abs(input_rfft.drop(), input_rfft_centered.drop(), 1e-4));

                auto input_fft = noa::random(noa::Normal<f32>{-5, 5}, shape, options);
                noa::signal::ctf_anisotropic_2d<"FC2FC">({}, result_fft, shape, ctf);
                auto input_fft_centered = noa::fft::remap_2d("F2FC", input_fft, shape);
                noa::ewise(noa::wrap(input_fft_centered, result_fft), input_fft_centered, noa::Multiply{});
                noa::signal::ctf_anisotropic_2d<"F2FC">(input_fft.copy(), input_fft, shape, ctf);
                REQUIRE(test::allclose_abs(input_fft.drop(), input_fft_centered.drop(), 1e-4));
            }
        }
    }
...
TEST_CASE("signal::ctf_anisotropic, default range, vs isotropic") {
    ...
    for (auto device: devices) {
...

        {
            ...
            noa::signal::ctf_anisotropic<"FC2FC">(input, output, shape, ctf_aniso, {.rank = 2});
            noa::signal::ctf_anisotropic<"FC2FC">({}, expected_0, shape, ctf_aniso, {.rank = 2});
            noa::signal::ctf_anisotropic<"FC2FC">(expected_1, shape, ctf_aniso, {.rank = 2});
            REQUIRE(test::allclose_abs(output, expected_0, 1e-4));
            REQUIRE(test::allclose_abs(output, expected_1, 1e-4));
        }

        {
            ...
            const auto anisotropic = noa::empty_like(isotropic);
            noa::signal::ctf_isotropic<"H2H">(isotropic, shape, ctf_iso, {.rank = 2});
            noa::signal::ctf_anisotropic<"H2H">(anisotropic, shape, ctf_aniso, {.rank = 2});
