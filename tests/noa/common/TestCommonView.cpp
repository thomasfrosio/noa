...
TEST_CASE("View") {
    ...
    // to const
    View<const int> d{a.data(), a.stride(), a.shape()};
    View<const int> e{a};
...
TEST_CASE("View - indexing") {
    ...
    const View<int> v0{buffer.get(), stride, shape};
    REQUIRE(v0(1, 45, 23, 10) == buffer[at(1, 45, 23, 10, stride)]);

    AND_THEN("full extent") {
        const View<int> v1 = v0.subview(ellipsis_t{}, 15);
        REQUIRE(all(v1.shape() == size4_t{3, 50, 40, 1}));
        REQUIRE(v0(0, 0, 0, 15) == v1(0));

        const View<int> v2 = v0.subview(ellipsis_t{}, 39, full_extent_t{});
        REQUIRE(all(v2.shape() == size4_t{3, 50, 1, 60}));
        REQUIRE(v0(1, 1, 39, 3) == v2(1, 1, 0, 3));

        const View<int> v3 = v0.subview(ellipsis_t{}, 9, full_extent_t{}, full_extent_t{});
        REQUIRE(all(v3.shape() == size4_t{3, 1, 40, 60}));
        REQUIRE(v0(1, 9, 25, 14) == v3(1, 0, 25, 14));

        const View<int> v4 = v0.subview(1, full_extent_t{}, full_extent_t{}, full_extent_t{});
        const View<int> v5 = v0.subview(1);
    ...
    AND_THEN("slice") {
        const View<int> v1 = v0.subview(ellipsis_t{}, slice_t{});
        REQUIRE(all(v1.shape() == v0.shape()));

        const View<int> v2 = v0.subview(ellipsis_t{}, slice_t{5, 45});
        const View<int> v3 = v0.subview(ellipsis_t{}, slice_t{5, -15});
        ...
        const View<int> v4 = v3.subview(slice_t{0, -1},
                                        slice_t{1, 100}, // clamp
                                        slice_t{10, 15},
                                        slice_t{-25, -10});
    ...
    AND_THEN("stride") {
        const View<int> v1 = v0.subview(ellipsis_t{}, slice_t{0, 60, 2});
    ...
    AND_THEN("empty or oob") {
        const View<int> v1 = v0.subview(slice_t{3, 3},
                                        slice_t{30, 20},
                                        slice_t{10, 10},
                                        slice_t{-10, -20});
        ...
        REQUIRE_THROWS(v0.subview(2, 30, 10, 60 /* oob */));
        REQUIRE_THROWS(v0.subview(2, 30, -41 /* oob */, 0));
    }
}
