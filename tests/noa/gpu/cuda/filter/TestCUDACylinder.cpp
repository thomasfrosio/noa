#include <noa/gpu/cuda/filter/Cylinder.h>
...
TEMPLATE_TEST_CASE("cuda::filter::cylinder(), contiguous", "[noa][cuda][filter]", float, double) {
    ...
    AND_THEN("INVERT = false") {
        ...
        cuda::filter::cylinder(d_mask.get(), shape, shifts, radius_xy, radius_z, taper, stream);
        cuda::memory::copy(d_mask.get(), h_cuda_mask.get(), d_mask.size(), stream);
        cpu::filter::cylinder(h_mask.get(), shape, shifts, radius_xy, radius_z, taper);
        ...
        cuda::filter::cylinder(d_data.get(), d_data.get(), shape, shifts, radius_xy, radius_z, taper, batches, stream);
        cuda::memory::copy(d_data.get(), h_cuda_data.get(), d_data.size(), stream);
        cpu::filter::cylinder(h_data.get(), h_data.get(), shape, shifts, radius_xy, radius_z, taper, batches);
        cuda::Stream::synchronize(stream);
    ...
    AND_THEN("INVERT = true") {
        ...
        // Test saving the mask.
        cuda::filter::cylinder<true>(d_mask.get(), shape, shifts, radius_xy, radius_z, taper, stream);
        cuda::memory::copy(d_mask.get(), h_cuda_mask.get(), d_mask.size(), stream);
        cpu::filter::cylinder<true>(h_mask.get(), shape, shifts, radius_xy, radius_z, taper);
        cuda::Stream::synchronize(stream);
        TestType diff = test::getAverageDifference(h_mask.get(), h_cuda_mask.get(), elements);
        REQUIRE_THAT(diff, test::isWithinAbs(float(0.), 1e-6));

        // Test on-the-fly, in-place.
        cuda::filter::cylinder<true>(d_data.get(), d_data.get(), shape, shifts, radius_xy, radius_z,
                                     taper, batches, stream);
        cuda::memory::copy(d_data.get(), h_cuda_data.get(), d_data.size(), stream);
        cpu::filter::cylinder<true>(h_data.get(), h_data.get(), shape, shifts, radius_xy, radius_z, taper, batches);
        cuda::Stream::synchronize(stream);
        diff = test::getAverageDifference(h_data.get(), h_cuda_data.get(), elements * batches);
        REQUIRE_THAT(diff, test::isWithinAbs(float(0.), 1e-6));
    }
...
TEMPLATE_TEST_CASE("cuda::filter::cylinder(), padded", "[noa][cuda][filter]", float, double) {
    ...
    AND_THEN("INVERT = false") {
        ...
        // Test saving the mask.
        cuda::filter::cylinder(d_mask.get(), d_mask.pitch(), shape, shifts, radius_xy, radius_z, taper, stream);
        ...
        // Test on-the-fly, in-place.
        cuda::filter::cylinder(d_data.get(), d_data.pitch(), d_data.get(), d_data.pitch(), shape,
    ...
    AND_THEN("INVERT = true") {
        ...
        // Test saving the mask.
        cuda::filter::cylinder<true>(d_mask.get(), d_mask.pitch(), shape, shifts, radius_xy, radius_z,
                                     taper, stream);
        cuda::memory::copy(d_mask.get(), d_mask.pitch(), h_cuda_mask.get(), shape.x, shape, stream);
        cpu::filter::cylinder<true>(h_mask.get(), shape, shifts, radius_xy, radius_z, taper);
        cuda::Stream::synchronize(stream);
        ...
        // Test on-the-fly, in-place.
        cuda::filter::cylinder<true>(d_data.get(), d_data.pitch(), d_data.get(), d_data.pitch(), shape,
                                     shifts, radius_xy, radius_z, taper, batches, stream);
        cuda::memory::copy(d_data.get(), d_data.pitch(), h_cuda_data.get(), shape.x, shape_batched, stream);
        cpu::filter::cylinder<true>(h_data.get(), h_data.get(), shape, shifts, radius_xy, radius_z, taper, batches);
        cuda::Stream::synchronize(stream);
