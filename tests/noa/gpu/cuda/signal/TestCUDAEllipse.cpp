...
TEST_CASE("cuda::signal::ellipse(), 2D", "[assets][noa][cuda]") {
    ...
    for (size_t nb = 0; nb < tests.size(); ++nb) {
        ...
        cuda::signal::ellipse<float>(nullptr, {}, mask_result.share(), strides, shape,
                                     center, radius, taper, math::inverse(fwd_transform),
                                     math::multiply_t{}, invert, stream);
        stream.synchronize();
        REQUIRE(test::Matcher(test::MATCH_ABS_SAFE, mask_expected.get(), mask_result.get(), elements, 1e-5));
        ...
        AND_THEN("invert = true") {
            ...
            cuda::signal::ellipse(input_result.share(), strides, input_result.share(), strides, shape,
                                  center, radius, taper, math::inverse(fwd_transform),
            ...
            stream.synchronize();
            REQUIRE(test::Matcher(test::MATCH_ABS_SAFE, input_result.get(), input_expected.get(), elements, 1e-5));
        }
...
TEMPLATE_TEST_CASE("cuda::signal::ellipse(), 2D matches 3D", "[assets][noa][cuda]",
    ...
    cuda::signal::ellipse(input.share(), strides, output_2d.share(), strides, shape,
                          center_2d, radius_2d, edge_size, math::inverse(fwd_transform_2d),
                          math::multiply_t{}, invert, stream);
    cuda::signal::ellipse(input.share(), strides, output_3d.share(), strides, shape,
                          center_3d, radius_3d, edge_size, math::inverse(fwd_transform_3d),
                          math::multiply_t{}, invert, stream);

    stream.synchronize();
    REQUIRE(test::Matcher(test::MATCH_ABS_SAFE, output_2d.get(), output_3d.get(), elements, 5e-5));
}

TEMPLATE_TEST_CASE("cuda::signal::ellipse(), vs cpu", "[assets][noa][cuda]",
                   float, double, cfloat_t, cdouble_t) {
    ...
    cpu::signal::ellipse(input.share(), strides, output_cpu.share(), strides, shape,
                         center, radius, edge_size, math::inverse(fwd_transform),
                         math::multiply_t{}, invert, cpu_stream);
    cpu_stream.synchronize();
    cuda::signal::ellipse(input.share(), strides, output_cuda.share(), strides, shape,
                          center, radius, edge_size, math::inverse(fwd_transform),
                          math::multiply_t{}, invert, gpu_stream);
    gpu_stream.synchronize();
    REQUIRE(test::Matcher(test::MATCH_ABS_SAFE, output_cpu.get(), output_cuda.get(), elements, 1e-3));
}
