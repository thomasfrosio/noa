...
TEMPLATE_TEST_CASE("cuda::signal::cylinder(), padded", "[noa][cuda][filter]", half_t, float, double) {
    ...
    AND_THEN("INVERT = false") {
        ...
        cuda::signal::cylinder<false, TestType>(nullptr, {}, d_mask.share(), d_mask.stride(), shape,
        ...
        cuda::signal::cylinder<false, TestType>(d_data.share(), d_data.stride(), d_data.share(), d_data.stride(), shape,
                                      center, radius, length, taper, gpu_stream);
    ...
    AND_THEN("INVERT = true") {
        ...
        // Test saving the mask.
        cuda::signal::cylinder<true, TestType>(nullptr, {}, d_mask.share(), d_mask.stride(), shape,
                                               center, radius, length, taper, gpu_stream);
        cuda::memory::copy<TestType>(d_mask.share(), d_mask.stride(),
                                     h_cuda_mask.share(), stride,
                                     d_mask.shape(), gpu_stream);
        ...
        // Test on-the-fly, in-place.
        cuda::signal::cylinder<true, TestType>(d_data.share(), d_data.stride(), d_data.share(), d_data.stride(), shape,
                                               center, radius, length, taper, gpu_stream);
