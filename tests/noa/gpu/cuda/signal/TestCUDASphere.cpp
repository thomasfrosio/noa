...
TEST_CASE("cuda::signal::sphere(), 2D", "[assets][noa][cuda]") {
    test::Randomizer<float> randomizer(-5, 5);

    const path_t path_base = test::NOA_DATA_PATH / "signal";
    YAML::Node tests = YAML::LoadFile(path_base / "tests.yaml")["sphere"]["tests2D"];
    io::MRCFile file;
    cuda::Stream stream;
    ...
    for (size_t nb = 0; nb < tests.size(); ++nb) {
        ...
        cuda::signal::sphere<float>(nullptr, {}, mask_result.share(), strides, shape,
        ...
        AND_THEN("invert = true") {
            ...
            cuda::signal::sphere(input_result.share(), strides, input_result.share(), strides, shape,
...
TEST_CASE("cuda::signal::sphere(), 3D", "[assets][noa][cuda]") {
    test::Randomizer<float> randomizer(-5, 5);

    const path_t path_base = test::NOA_DATA_PATH / "signal";
    YAML::Node tests = YAML::LoadFile(path_base / "tests.yaml")["sphere"]["tests3D"];
    io::MRCFile file;
    cuda::Stream stream;
...
TEMPLATE_TEST_CASE("cuda::signal::sphere(), 2D matches 3D", "[assets][noa][cuda]",
    ...
    const float angle = math::deg2rad(-67.5f);
    ...
    cuda::signal::sphere(input.share(), strides, output_2d.share(), strides, shape,
                         center_2d, radius, edge_size, math::inverse(fwd_transform_2d), invert, stream);
    cuda::signal::sphere(input.share(), strides, output_3d.share(), strides, shape,
                         center_3d, radius, edge_size, math::inverse(fwd_transform_3d), invert, stream);

    stream.synchronize();
    REQUIRE(test::Matcher(test::MATCH_ABS_SAFE, output_2d.get(), output_3d.get(), elements, 1e-5));
}

TEMPLATE_TEST_CASE("cuda::signal::sphere(), vs cpu", "[assets][noa][cuda]",
                   float, double, cfloat_t, cdouble_t) {
    ...
    cpu::signal::sphere(input.share(), strides, output_cpu.share(), strides, shape,
                        center, radius, edge_size, math::inverse(fwd_transform), invert, cpu_stream);
    cpu_stream.synchronize();
    cuda::signal::sphere(input.share(), strides, output_cuda.share(), strides, shape,
