...
TEST_CASE("cuda::signal::convolve()", "[assets][noa][cuda][filter]") {
    using namespace noa;
    ...
    for (size_t nb = 0; nb < tests.size(); ++nb) {
        ...
        cuda::signal::convolve<float, float>(data.share(), stride,
...
TEST_CASE("cuda::signal::convolve() - separable", "[assets][noa][cuda][filter]") {
    using namespace noa;
    ...
    for (size_t nb = 0; nb < tests.size(); ++nb) {
        ...
        cuda::memory::PtrManaged<float> result(data.elements(), stream);
        cuda::signal::convolve<float, float>(data.share(), stride, result.share(), stride, shape,
