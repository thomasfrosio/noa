...
TEMPLATE_TEST_CASE("cuda::transform::fft::shift(2|3)D()", "[noa][cuda][transform]", cfloat_t, cdouble_t) {
    ...
    cuda::memory::copy(h_input.share(), stride, d_input.share(), d_input.strides(), d_input.shape(), gpu_stream);
    ...
    if (ndim == 2) {
        ...
        switch (remap) {
            case fft::H2H: {
                ...
                cuda::signal::fft::shift2D<fft::H2H>(
                        d_input.share(), d_input.strides(), d_output.share(), d_output.strides(), shape,
    ...
    cuda::memory::copy(d_output.share(), d_output.strides(), h_output_cuda.share(), stride, d_output.shape(), gpu_stream);
