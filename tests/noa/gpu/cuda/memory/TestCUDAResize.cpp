    ...
    for (size_t nb = 0; nb < tests.size(); ++nb) {
        ...
        if (is_centered) { // with central pixel (N//2) set to 0
            ...
            for (uint batch = 0; batch < input_shape[0]; ++batch)
                h_input[indexing::at(batch, center[1], center[2], center[3], input_shape.strides())] = 0;
        }
        ...
        cuda::Stream stream;
        {
            ...
            if (is_centered) {
                cuda::memory::resize<float>(d_input.share(), input_shape.strides(), input_shape,
                                            d_output.share(), output_shape.strides(), output_shape,
                                            border_mode, border_value, stream);
            } else {
                cuda::memory::resize<float>(d_input.share(), input_shape.strides(), input_shape, left, right,
                                            d_output.share(), output_shape.strides(),
                                            border_mode, border_value, stream);
            }
...
TEMPLATE_TEST_CASE("cuda::memory::resize() - edge cases", "[noa][cuda][memory]",
    ...
    AND_THEN("copy") {
        ...
        cuda::memory::resize<TestType>(d_input.share(), shape.strides(), shape,
                                       d_output.share(), shape.strides(), shape,
