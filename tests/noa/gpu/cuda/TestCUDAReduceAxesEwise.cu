...
TEST_CASE("cuda::reduce_axes_ewise") {
    ...
    AND_THEN("sum over one axis") {
        ...
        for (auto& input_shape: input_shapes) {
            ...
            for (auto i: noa::irange(4)) {
                ...
                output_shape[i] = 1;
                const auto output_n_elements = output_shape.elements();
                ...
                REQUIRE(test::Matcher(test::MATCH_ABS, output_buffer.get(), expected_value, output_n_elements, 1e-5));
            }
        }
    }
    ...
    AND_THEN("sum per batch") {
        ...
        for (const auto& input_shape: input_shapes) {
            ...
            const auto input_n_elements = input_shape.elements();
            const auto output_n_elements = output_shape.elements();
            const f64 expected_value = static_cast<f64>(input_shape.pop_front().elements());
