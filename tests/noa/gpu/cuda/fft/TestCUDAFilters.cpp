...
TEMPLATE_TEST_CASE("cuda::fft::lowpass()", "[noa][cuda][fft]",
...
TEMPLATE_TEST_CASE("cuda::fft::lowpass(), remap", "[noa][cuda][fft]", half_t, float) {
...
TEMPLATE_TEST_CASE("cuda::fft::highpass()", "[noa][cuda][fft]",
                   half_t, float, double, chalf_t, cfloat_t, cdouble_t) {
    using real_t = noa::traits::value_type_t<TestType>;
...
TEMPLATE_TEST_CASE("cuda::fft::highpass(), remap", "[noa][cuda][fft]", half_t, float) {
    const size4_t shape = test::getRandomShapeBatched(3);
    const float cutoff = 0.4f;
    const float width = 0.1f;
...
TEMPLATE_TEST_CASE("cuda::fft::bandpass(), remap", "[noa][cuda][fft]", half_t, float) {
    const size4_t shape = test::getRandomShapeBatched(3);
