...
TEMPLATE_TEST_CASE("cuda::math:: generics - complex, contiguous", "[noa][cuda][math]", cfloat_t, cdouble_t) {
    ...
    AND_THEN("abs") {
        ...
        cpu::memory::PtrHost<real_t> h_results(elements);
        cpu::memory::PtrHost<real_t> h_expected(elements);
    ...
    AND_THEN("real, imag") {
        ...
        REQUIRE(test::Matcher(test::MATCH_ABS_SAFE, data.get(), cuda_results.get(), elements, 1e-6));
    }
