...
TEMPLATE_TEST_CASE("cuda::math::randomize() - padded", "[noa][cuda][math]", float, double) {
    ...
    cuda::math::randomize(math::uniform_t{}, data.share(), data.strides(), shape, value_t{-10}, value_t{10}, stream);
    TestType min = cuda::math::min(data.share(), data.strides(), shape, stream);
    TestType max = cuda::math::max(data.share(), data.strides(), shape, stream);
    TestType mean = cuda::math::mean(data.share(), data.strides(), shape, stream);
    REQUIRE(min >= value_t{-10});
    REQUIRE(max <= value_t{10});
    REQUIRE_THAT(mean, Catch::WithinAbs(0, 0.1));
